...
Special polar/Greenland support source:
...
- For polar tiles (`north edge <= 60°S` or `south edge >= 75°N`) and all Greenland product tiles, the land mask is stricter:
...
- If a special polar/Greenland WT tile is missing, S2 ocean plus S2 white exclusion is used as the fallback and recorded in `source`.
...
- `WT_S2_WHITE`: special polar/Greenland WT ocean exclusion plus S2 white exclusion.
- `S2_WHITE_FALLBACK`: special polar/Greenland fallback when matching WT is missing.
...
- Antarctica, high Arctic, and Greenland are not automatically free by latitude. They become free or cheaper only when the WT/S2-white land mask produces zero or low billable land area.
