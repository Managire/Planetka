...
async function invalidateAnalyticsSnapshots(_env) {
  // Commerce invalidates pricing data directly. Analytics snapshots are isolated in planetka-analytics.
}
...
async function ensureCreditTables(db) {
  ...
  await ensureUserProductQuoteTables(db);
  await dbRun(
    db,
    `
...
async function ensureUserProductQuoteTables(db) {
  await dbRun(
    db,
    `
  ...
  await dbRun(
    db,
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_user_product_quotes_quote_id ON user_product_quotes(quote_id)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quotes_user_status ON user_product_quotes(user_id, status, updated_at DESC)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quotes_product ON user_product_quotes(product_id, catalog_version, status)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quotes_versions ON user_product_quotes(user_id, pricing_version, entitlement_version, catalog_version)`,
  );
  await dbRun(
    db,
    `
  ...
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_batches_status ON user_product_quote_batches(status, created_at)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_batches_user ON user_product_quote_batches(user_id, created_at DESC)`,
  );
  await dbRun(
    db,
    `
  ...
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_jobs_queue ON user_product_quote_jobs(status, available_at, job_round, priority, created_at)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_jobs_queue_round ON user_product_quote_jobs(status, job_round, priority, available_at, created_at)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_jobs_user_product ON user_product_quote_jobs(user_id, product_id, catalog_version, status)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_jobs_batch ON user_product_quote_jobs(batch_id, job_round, status)`,
  );
  await dbRun(
    db,
    `CREATE INDEX IF NOT EXISTS idx_user_product_quote_jobs_lock ON user_product_quote_jobs(lock_token, locked_at)`,
  );
  await dbRun(
    db,
    `
...
async function dispatchCommerceRoute(request, env, path) {
  ...
  switch (path) {
    ...
    case "/credits/account-page-link":
      if (request.method === "POST") return await handleCreditAccountPageLinkRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/account":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditAccountPageRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/estimate":
      if (request.method === "POST") return await handleCreditEstimateRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/checkout":
      if (request.method === "POST") return await handleCreditCheckoutRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/region-offers":
      if (request.method === "POST") return await handleCreditRegionOffersRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/region-pack-related-offers":
      if (request.method === "POST") return await handleCreditRegionPackRelatedOffersRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/region-pack-detail-link":
      if (request.method === "POST") return await handleCreditRegionPackDetailLinkRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/scene-detail-link":
      if (request.method === "POST") return await handleCreditSceneDetailLinkRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/scene-map":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditSceneMapRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/scene-checkout":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditSceneCheckoutRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    ...
    case "/credits/region-pack-catalog":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditRegionPackCatalogRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/region-pack-catalog-page":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditRegionPackCatalogPageRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/region-pack-checkout":
      if (request.method === "GET" || request.method === "HEAD" || request.method === "POST") return await handleCreditRegionPackCheckoutFromTokenRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/payment-success":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditPaymentSuccessRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/payment-cancelled":
      if (request.method === "GET" || request.method === "HEAD") return await handleCreditPaymentCancelledRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/purchase-history":
      if (request.method === "GET") return await handleCreditPurchaseHistoryRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/licenced-download-report":
      if (request.method === "POST") return await handleCreditLicencedDownloadReportRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/credits/unlocked":
      if (request.method === "GET") return await handleCreditUnlockedRoute(request, env, TILE_ROUTE_DEPS);
      return null;
    case "/stripe/webhook":
...
async function dispatchRequest(request, env, path) {
  const match = await dispatchCommerceRoute(request, env, path);
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const response = await dispatchRequest(request, env, path);
      maybeKickProductQuoteQueue(ctx, env, path, request.url);
      return response;
    } catch (error) {
      console.error("commerce_worker.request.error", JSON.stringify({ path, method: request.method, error: String(error && error.message || "internal_error") }));
