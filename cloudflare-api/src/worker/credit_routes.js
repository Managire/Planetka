...
import { CHECKOUT_PLANET_JPEG_BASE64 } from "./checkout_planet.generated.js";
...
const DEFAULT_SCENE_CUSTOM_LICENCE_FEE_EUR = 15.00;
...
const REGION_PACK_MAP_ASSET_REVISION = `${REGION_PACK_CATALOG_VERSION}:outline-v4-product-bg-wt-blue-v4-partial-dateline-v7-admin-labels-v1-success-upsell-v1-catalog-flat-v1-price-breakdown-v1-hover-breakdown-v1-summary-partial-v1-pricing-runtime-v5-runtime-buckets-v1-canonical-pricing-v2-coeff-km2-v1-post-purchase-new-v1-button-border-gold-v1-tile-tooltip-v5-immediate-static-js-v10-product-outlines-v1-on-demand-map-v4-incremental-chunks-v1-geo-bg-v1-soft-red-v1-scene-map-v1-tooltip-cardinal-v2-product-bg-frame-v1-white-bg-v2-mini-upsell-tiles-v2-product-map-d-equals-z-v1-chunk1000-v2-wt-tile-bg-v2-scene-page-v3-map-loading-v6-live-bounds-nofallback-bg-v1-mini-square-bg-v1-world-bg-align-v1-mini-cover-v1-tile-padded-bounds-v1-world-no-pad-v1-global-borders-v2-us-ca-bounds-v1-mini-product-outlines-v1-adm1-outline-v3-mini-native-frame-v1-china-bounds-v1-east-asia-bounds-v1-world-mini-full-v1-asia-bg-display-v1-free-visible-v1-map-random-puzzle-v1-world-z2-ready-v1-asia-bg-frame-v1-subdivision-line-v1-fast-reveal-v1-hover-highlight-v1-map-worker-v1-graceful-map-failure-v2-page-resilience-v1-context-hit-area-v1-asia-mini-frame-v1-world-frame-z2-default-v1-no-tile-animation-v1-mini-after-main-v1-aus-regions-frame-v1-map-zoom-buttons-v1-other-options-table-v2-wide-numbers-v1-card-breaks-v1-sticky-head-v1-narrow-label-actions-v2-catalog-sticky-bar-v1-world-z3-client-v1`;
const REGION_PACK_PRICING_ENGINE_REVISION = "d1-complete-map-state-v3";
const ACCOUNT_COUNTRY_BORDERS_ASSET_REVISION = `${REGION_PACK_CATALOG_VERSION}:account-country-borders-v2`;
...
function regionPackTileEntriesInRowOrder(rows) {
  return (Array.isArray(rows) ? rows : [])
    .map(regionPackTileEntryFromRow)
    .filter(Boolean);
}
...
async function regionPackTileRowsForProductAfterCursor(db, product, deps, options = {}) {
  ...
  return regionPackTileEntriesInRowOrder(rows || []);
}
...
function formatEur(value) {
  return `€${normalizeCreditAmount(value).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}
...
function dataSizeBytesForProduct(product) {
  return Math.max(0, Number.parseInt(product && product.data_size_bytes || 0, 10) || 0);
}
...
function formatDataSize(bytes) {
  ...
  return `${(value / 1_000_000_000_000).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TB`;
}
...
function defaultCheckoutProductImageUrl(env) {
  return String(
    env.STRIPE_CHECKOUT_PRODUCT_IMAGE_URL
    || env.PLANETKA_CHECKOUT_PRODUCT_IMAGE_URL
    || "https://api.planetka.io/credits/page-assets/checkout-frisco4k.jpg",
  ).trim();
}
...
function regionProductShowsSubdivisionOutline(product) {
  const adm1Codes = Array.isArray(product && product.adm1_codes) ? product.adm1_codes : [];
  if (!adm1Codes.length) {
    return false;
  }
  const adm0Codes = new Set((Array.isArray(product && product.adm0_codes) ? product.adm0_codes : []).map((code) => String(code || "").trim().toUpperCase()));
  return adm0Codes.has("USA") || adm0Codes.has("CAN") || adm0Codes.has("CHN");
}
...
function regionProductPublicPayload(product) {
  ...
  const bbox = Array.isArray(product.map_bounds)
    ? product.map_bounds.map((value) => Number(value))
    : (Array.isArray(product.bbox) ? product.bbox.map((value) => Number(value)) : []);
  ...
  return {
    ...
    included_area_links: regionProductIncludedAreaLinks(product),
    adm1_codes: Array.isArray(product.adm1_codes) ? product.adm1_codes : [],
    show_subdivision_outline: regionProductShowsSubdivisionOutline(product),
    data_size_bytes: dataSizeBytesForProduct(product),
    data_size_text: formatDataSize(dataSizeBytesForProduct(product)),
    bounds,
  };
}
...
async function regionProductPricingSummariesD1(db, products, deps) {
  const safeProducts = (Array.isArray(products) ? products : [])
    .filter((product) => product && typeof product === "object");
  const productIds = Array.from(new Set(safeProducts
    .map((product) => String(product.id || "").trim().toLowerCase())
    .filter(Boolean)));
  ...
  const rows = [];
  for (const chunk of fixedSizeChunks(productIds, SQL_VARIABLE_SAFE_CHUNK_SIZE)) {
    rows.push(...await deps.dbAll(
      db,
      `
        SELECT
        ...
        FROM region_pack_tile_entries
        WHERE catalog_version = ?
          AND region_pack_id IN (${chunk.map(() => "?").join(",")})
  ...
  for (const row of rows) {
    ...
    boundedCacheSet(
      REGION_PRODUCT_PRICING_SUMMARY_CACHE,
      [REGION_PACK_CATALOG_VERSION, productId, coefficient.toFixed(6)].join("|"),
      summary,
      512,
    );
  }
...
export async function listRegionProductPricingRows(env = {}, deps = {}) {
  ...
  const products = REGION_PRODUCTS.filter((entry) => entry && !isHiddenRegionProduct(entry));
  const summaries = db ? await regionProductPricingSummariesD1(db, products, deps) : new Map();
  const rows = [];
  for (const product of products) {
    const productId = String(product && product.id || "").trim().toLowerCase();
    const summary = summaries.get(productId) || regionProductPricingSummary(product);
...
async function invalidateUserEntitlementCaches(db, userId, deps) {
  const safeUserId = String(userId || "").trim();
  if (!safeUserId) {
    return;
  }
...
const WORLD_FRAME_REGION_PACK_IDS = new Set(["asia", "oceania", "pacific_islands", "russia"]);
const WORLD_FRAME_DEFAULT_ZOOM2_REGION_PACK_IDS = new Set(["asia", "oceania", "pacific_islands", "russia"]);
...
function includedAreaLabelKey(label) {
  return String(label || "").trim().replace(/\s+/g, " ").toLowerCase();
}

function visibleRegionProductByName(label) {
  const key = includedAreaLabelKey(label);
  if (!key) {
    return null;
  }
  const matches = REGION_PRODUCTS.filter((product) => (
    product
    && !isHiddenRegionProduct(product)
    && includedAreaLabelKey(product.name) === key
  ));
  return matches.length === 1 ? matches[0] : null;
}

function visibleRegionProductByAdm0Code(adm0Code) {
  const code = String(adm0Code || "").trim().toUpperCase();
  if (!code) {
    return null;
  }
  const matches = REGION_PRODUCTS.filter((product) => (
    product
    && !isHiddenRegionProduct(product)
    && Array.isArray(product.adm0_codes)
    && product.adm0_codes.map((entry) => String(entry || "").trim().toUpperCase()).includes(code)
  ));
  return matches.find((product) => (
    String(product.type || "").trim().toLowerCase() === "country"
    && !(Array.isArray(product.adm1_codes) && product.adm1_codes.length)
  ))
    || matches.find((product) => String(product.type || "").trim().toLowerCase() === "macro_region")
    || matches[0]
    || null;
}

function regionProductIncludedAreaLinks(product) {
  const labels = regionProductIncludedCountries(product);
  const allowedKeys = new Set(labels.map(includedAreaLabelKey));
  const links = new Map();
  const addLink = (label, productId) => {
    const safeLabel = String(label || "").trim();
    const safeId = String(productId || "").trim();
    const key = includedAreaLabelKey(safeLabel);
    if (!safeLabel || !safeId || !allowedKeys.has(key)) {
      return;
    }
    const target = regionProductById(safeId);
    if (!target || isHiddenRegionProduct(target)) {
      return;
    }
    links.set(safeLabel, safeId);
  };

  for (const label of labels) {
    const directProduct = visibleRegionProductByName(label);
    if (directProduct) {
      addLink(label, directProduct.id);
    }
  }

  const id = String(product && product.id || "").trim();
  if (id === "oceania") {
    addLink("Australia", "australia");
  }
  for (const codeRaw of Array.isArray(product && product.adm0_codes) ? product.adm0_codes : []) {
    const code = String(codeRaw || "").trim().toUpperCase();
    const label = String(PACIFIC_INCLUDED_AREA_LABEL_BY_CODE.get(code) || DISPLAY_AREA_LABEL_BY_ADM0_CODE.get(code) || "").trim();
    const target = visibleRegionProductByAdm0Code(code);
    if (label && target) {
      addLink(label, target.id);
    }
  }

  const generated = GENERATED_REGION_PACK_DETAILS[id];
  if (generated && Array.isArray(generated.countries)) {
    for (const entry of generated.countries) {
      const code = String(entry && entry.GID_0 || "").trim().toUpperCase();
      const rawLabel = String(DISPLAY_AREA_LABEL_BY_ADM0_CODE.get(code) || entry && (entry.NAME_1 || entry.name || entry.COUNTRY) || "").trim();
      const label = CHINA_ADMIN_REGION_LABELS.has(rawLabel.toLowerCase()) ? "China" : rawLabel;
      const target = code === "CHN" ? regionProductById("china") : visibleRegionProductByAdm0Code(code);
      if (label && target) {
        addLink(label, target.id);
      }
    }
  ...
  for (const countryId of Array.isArray(product && product.countries) ? product.countries : []) {
    const child = regionProductById(countryId);
    if (!child) {
      continue;
    }
    const parentLabel = adminRegionParentLabelForProduct(child);
    if (parentLabel) {
      const parentProduct = visibleRegionProductByName(parentLabel);
      if (parentProduct) {
        addLink(parentLabel, parentProduct.id);
      }
      continue;
    }
    if (CHINA_ADMIN_REGION_LABELS.has(String(child.name || "").trim().toLowerCase())) {
      addLink("China", "china");
      continue;
    }
    if (!isHiddenRegionProduct(child)) {
      addLink(child.name, child.id);
    }
  }

  const result = {};
  for (const label of labels) {
    const productId = links.get(label);
    if (productId) {
      result[label] = productId;
    }
  }
  return result;
}
...
function regionPackMapProductId(productOrId) {
  if (typeof productOrId === "string") {
    return String(productOrId || "").trim().toLowerCase();
  }
  return String(productOrId && productOrId.id || "").trim().toLowerCase();
}

function regionPackPrefersWorldFrameZoom2(productOrId) {
  return WORLD_FRAME_DEFAULT_ZOOM2_REGION_PACK_IDS.has(regionPackMapProductId(productOrId));
}

function preferredWorldFrameMapDefaultLevel(productOrId) {
  const productId = regionPackMapProductId(productOrId);
  if (productId === "world") {
    return 3;
  }
  if (WORLD_FRAME_DEFAULT_ZOOM2_REGION_PACK_IDS.has(productId)) {
    return 2;
  }
  return 0;
}

function preferredRegionPackMapDefaultLevel(productOrId, levelCounts, maxInitialTiles = REGION_PACK_MAP_LEVEL_CHUNK_LIMIT) {
  const counts = levelCounts && typeof levelCounts === "object" ? levelCounts : {};
  const levels = Object.keys(counts)
    .map((level) => Number.parseInt(level, 10))
    .filter((level) => isRegionPackPaidZLevel(level) && Math.max(0, Number.parseInt(counts[String(level)] || 0, 10) || 0) > 0)
    .sort((a, b) => a - b);
  const preferredWorldFrameLevel = preferredWorldFrameMapDefaultLevel(productOrId);
  if (preferredWorldFrameLevel && levels.includes(preferredWorldFrameLevel)) {
    return preferredWorldFrameLevel;
  }
  return defaultRegionPackMapLevel(counts, maxInitialTiles);
}

function preferredReadyRegionPackMapDefaultLevel(productOrId, levels, levelCounts = null) {
  const available = (Array.isArray(levels) ? levels : [])
    .map((level) => Number.parseInt(level, 10))
    .filter((level) => isRegionPackPaidZLevel(level))
    .sort((a, b) => a - b);
  const preferredWorldFrameLevel = preferredWorldFrameMapDefaultLevel(productOrId);
  if (preferredWorldFrameLevel && available.includes(preferredWorldFrameLevel)) {
    return preferredWorldFrameLevel;
  }
  if (available.includes(1)) {
    return 1;
  }
  if (levelCounts && typeof levelCounts === "object") {
    return defaultRegionPackMapLevel(levelCounts);
  }
  return available.length ? available[0] : 0;
}

async function buildRegionPackOnDemandMapState(db, product, quote, deps) {
  if (!product || !quote) {
    return null;
  }
  ...
  return {
    ...
    included_countries: regionProductIncludedCountries(product),
    levels,
    default_level: preferredRegionPackMapDefaultLevel(product, levelCounts),
...
main{max-width:1500px;margin:0 auto;padding:24px}h1{margin:0 0 8px;font-size:28px;font-weight:650}.page-heading{display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap;margin-bottom:8px}.catalog-back{display:inline-flex;align-items:center;margin-top:2px;padding:8px 12px;border:1px solid var(--line);border-radius:9px;background:var(--secondary-btn);color:var(--text);font-weight:750;text-decoration:none;white-space:nowrap}.catalog-back:hover{border-color:var(--accent);color:var(--accent)}.page-heading h1{flex:1;min-width:260px;margin:0}.muted{color:var(--muted)}
.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:18px 0}.card{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:12px}.card b{display:block;font-size:22px;margin-top:4px;white-space:pre-line}.card.final-price{border-color:#8f732f;box-shadow:0 0 0 1px rgba(217,164,65,.16) inset}.card.final-price b{font-size:26px}.buy-now{width:100%;font-size:16px}
...
select,input{background:var(--input);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:7px 10px}svg{width:100%;height:auto;background:var(--map-bg);border:1px solid var(--line);border-radius:10px}#map .map-tile{transition:filter .08s ease-out,opacity .08s ease-out}#map .map-tile:hover{filter:saturate(2.2) brightness(1.18);opacity:.86}.map-shell{position:relative}.map-toolbar-overlay{position:absolute;top:10px;left:10px;right:10px;z-index:3;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:8px 10px;border:1px solid rgba(255,255,255,.22);border-radius:10px;background:rgba(7,10,16,.68);color:#fff;backdrop-filter:blur(8px);box-shadow:0 6px 18px rgba(0,0,0,.22)}.map-toolbar-label{font-weight:400}.map-summary{color:#e6edf8;font-weight:400}.zoom-buttons{display:inline-flex;gap:5px;flex-wrap:wrap}.zoom-button{min-width:30px;height:28px;border:1px solid rgba(255,255,255,.35);border-radius:7px;background:rgba(255,255,255,.1);color:#fff;font:inherit;font-weight:750;line-height:1;cursor:pointer}.zoom-button.active{background:var(--button-accent);border-color:var(--button-accent);color:var(--button-text)}.zoom-button:disabled{opacity:.45;cursor:not-allowed}@media (prefers-color-scheme:light){.map-toolbar-overlay{background:rgba(255,255,255,.82);color:#1f252d;border-color:rgba(31,37,45,.16)}.map-summary{color:#475467}.zoom-button{background:rgba(31,37,45,.06);border-color:rgba(31,37,45,.22);color:#1f252d}.zoom-button.active{background:var(--button-accent);border-color:var(--button-accent);color:var(--button-text)}}.map-loading-overlay{position:absolute;inset:0;z-index:2;display:grid;place-items:center;pointer-events:none;font-weight:750;color:#fff;text-shadow:0 1px 3px rgba(0,0,0,.65)}.map-loading-overlay span{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:999px;background:rgba(7,10,16,.72);border:1px solid rgba(255,255,255,.25)}.map-loading-overlay span::before{content:"";width:13px;height:13px;border-radius:50%;border:2px solid rgba(255,255,255,.35);border-top-color:#fff;animation:planetka-spin .75s linear infinite}@keyframes planetka-spin{to{transform:rotate(360deg)}}.map-loading-overlay.hidden{display:none}.map-loading-overlay.failed{pointer-events:auto;text-shadow:none}.map-loading-overlay.failed span{display:flex;flex-direction:column;align-items:center;gap:8px;max-width:360px;border-radius:14px;text-align:center;background:rgba(13,17,24,.88);padding:16px 18px}.map-loading-overlay.failed span::before{display:none}.map-loading-overlay.failed em{font-style:normal;font-weight:500;color:#d7dde6}.map-retry{border:1px solid rgba(255,255,255,.32);border-radius:8px;background:var(--button-accent);color:var(--button-text);font:inherit;font-weight:750;padding:7px 12px;cursor:pointer}
...
.countries{display:flex;flex-wrap:wrap;gap:6px 10px;align-items:center}.countries div{display:inline-flex;align-items:center}.countries div:not(:last-child)::after{content:"·";margin-left:10px;color:var(--muted)}.countries a,.product-licence-links a{color:var(--accent);font-weight:650;text-decoration:none}.countries a:hover,.product-licence-links a:hover{text-decoration:underline}.product-licence-links{display:flex;flex-wrap:wrap;gap:8px 14px;margin-top:10px}.small{font-size:13px}.error{color:#d24533}
...
.upsells{overflow:visible}.upsells table{width:100%;min-width:760px;table-layout:auto;border-collapse:separate;border-spacing:0;background:var(--subpanel);border:1px solid var(--line);border-radius:10px;overflow:visible}.upsells th,.upsells td{padding:8px 10px;border-bottom:1px solid var(--table-line);text-align:right;white-space:nowrap;overflow:visible;text-overflow:clip}.upsells th:first-child,.upsells td:first-child{text-align:left;max-width:260px;overflow:hidden;text-overflow:ellipsis}.upsells td:first-child a{color:var(--text);font-weight:750;text-decoration:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}.upsells td:first-child a:hover{color:var(--accent)}.upsells tr:last-child td{border-bottom:0}.upsells thead{position:sticky;top:0;z-index:30}.upsells th{position:sticky;top:0;z-index:31;color:var(--text);background:var(--table-head);font-weight:650;box-shadow:0 1px 0 var(--table-line),0 8px 14px rgba(0,0,0,.18)}.upsells thead th:first-child{border-top-left-radius:9px}.upsells thead th:last-child{border-top-right-radius:9px}.saving{color:#9dd18d}.price{font-weight:700;color:var(--accent)}.pending{color:var(--muted);font-weight:650}.catalog-actions{overflow:visible;text-overflow:clip;white-space:nowrap}.catalog-actions .button{margin-left:6px}.catalog-actions .button:first-child{margin-left:0}
.button{display:inline-flex;align-items:center;justify-content:center;margin-top:10px;padding:9px 12px;border-radius:8px;background:var(--button-accent);color:var(--button-text,#111);text-decoration:none;font-weight:700}.button.secondary{margin-left:8px;background:var(--secondary-btn);color:var(--text);border:1px solid var(--line)}.button.disabled{opacity:.52;cursor:not-allowed;pointer-events:none}`;
...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:15px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
main{max-width:1500px;margin:0 auto;padding:24px}h1{margin:0 0 8px;font-size:28px;font-weight:650}h2{margin:22px 0 10px}.muted{color:var(--muted)}
.panel{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:14px;margin-top:14px;overflow:visible}.toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:12px 0}
...
table{width:100%;min-width:760px;table-layout:auto;border-collapse:separate;border-spacing:0;background:var(--subpanel);border:1px solid var(--line);border-radius:10px;overflow:visible}th,td{padding:8px 10px;border-bottom:1px solid var(--table-line);text-align:right;white-space:nowrap;overflow:visible;text-overflow:clip}th:first-child,td:first-child{text-align:left;max-width:260px;overflow:hidden;text-overflow:ellipsis}td:first-child a{color:var(--text);font-weight:750;text-decoration:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}td:first-child a:hover{color:var(--accent)}tr:last-child td{border-bottom:0}thead{position:sticky;top:0;z-index:30}th{position:sticky;top:0;z-index:31;color:var(--text);background:var(--table-head);font-weight:650;box-shadow:0 1px 0 var(--table-line),0 8px 14px rgba(0,0,0,.18)}thead th:first-child{border-top-left-radius:9px}thead th:last-child{border-top-right-radius:9px}.catalog-group-row td{position:relative;z-index:1;text-align:left;background:var(--panel);color:var(--text);font-size:20px;font-weight:750;padding-top:18px;padding-bottom:10px}.catalog-group-row:first-child td{padding-top:10px}.small{font-size:13px}.saving{color:var(--saving)}.price{font-weight:700;color:var(--price)}.pending{color:var(--muted);font-weight:650}.catalog-actions{overflow:visible;text-overflow:clip;white-space:nowrap}.catalog-actions .button{margin-left:6px}.catalog-actions .button:first-child{margin-left:0}
.button{display:inline-flex;align-items:center;justify-content:center;padding:7px 10px;border-radius:8px;background:var(--button-accent);color:var(--button-text,#111);text-decoration:none;font-weight:700}.button.secondary{background:var(--secondary-btn);color:var(--text);border:1px solid var(--line)}.button.disabled{opacity:.55;pointer-events:none;filter:grayscale(.25)}.empty{padding:12px;color:var(--muted)}.load-status{display:inline-grid;grid-template-columns:auto 6ch auto 6ch;column-gap:4px;align-items:baseline;font-variant-numeric:tabular-nums}.load-status .loaded{text-align:right}.load-status .total{text-align:left}.spinner{display:inline-block;width:14px;height:14px;border:2px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}`;
...
const ACCOUNT_PAGE_MAP_JS = `(() => {
  ...
  const bordersUrl = "/maps/account-country-borders.json?v=" + encodeURIComponent(String(DATA.country_borders_revision || DATA.map_asset_revision || Date.now()));
  ...
  function render(level) {
    ...
    svg.appendChild(el("image", {
      href: "/maps/region-pack-map-background.jpg?v=" + encodeURIComponent(String(DATA.map_asset_revision || Date.now())),
      x: 0,
      y: 0,
      width: 360,
      height: 180,
...
const fmtCents=(v)=>"€"+fmtMoney(int(v)/100);
const fmtDataSize=(v)=>{const b=int(v);const mb=b/1000000;if(mb<1000)return fmtMoney(mb)+" MB";const gb=b/1000000000;if(gb<1000)return fmtMoney(gb)+" GB";return fmtMoney(b/1000000000000)+" TB"};
const assetCache=new Map();
const assetVersion=encodeURIComponent(String(DATA.map_asset_revision||DATA.catalog_version||DATA.token||Date.now()));
const MAP_BG="/maps/region-pack-map-background.jpg?v="+assetVersion;
const outlinesUrl=(id)=>"/maps/region-pack-map-outlines?region_pack_id="+encodeURIComponent(String(id||""))+"&v="+assetVersion;
const miniOutlinesUrl="/maps/account-country-borders.json?v="+assetVersion;
...
const WORLD_FRAME_MAP_IDS=new Set(["world","asia","oceania","pacific_islands","russia"]);
const WORLD_FRAME_ZOOM2_DEFAULT_IDS=new Set(["asia","oceania","pacific_islands","russia"]);
const PACIFIC_MINI_FRAME_IDS=new Set(["oceania","pacific_islands"]);
const MINI_FRAME_BY_ID={
  asia:{min_lon:25,min_lat:-15,max_lon:155,max_lat:78},
  russia:{min_lon:-20,min_lat:-40,max_lon:200,max_lat:90}
};
const FULL_WORLD_BOUNDS={min_lon:-180,min_lat:-90,max_lon:180,max_lat:90};
function isWorldFrameMapId(id){return WORLD_FRAME_MAP_IDS.has(String(id||"").trim().toLowerCase())}
function forceWorldFrameBounds(asset){const id=asset&&asset.region_pack&&asset.region_pack.id;return isWorldFrameMapId(id)||String(id||"").trim().toLowerCase()==="world"?{...asset,bounds:FULL_WORLD_BOUNDS}:asset}
function mergeRegionPack(base,override){const merged={...(base||{}),...(override||{})};if(base&&base.show_subdivision_outline)merged.show_subdivision_outline=true;if(!Array.isArray(merged.adm1_codes)&&Array.isArray(base&&base.adm1_codes))merged.adm1_codes=base.adm1_codes;return merged}
function productMapBgHref(id){const raw=String(id||"").trim().toLowerCase();if(WORLD_FRAME_MAP_IDS.has(raw))return MAP_BG;const safe=encodeURIComponent(raw);return safe?"/maps/region-pack-map-background.jpg?region_pack_id="+safe+"&v="+assetVersion:""}
...
function includedAreaLinkId(label){const links=DATA.included_area_links&&typeof DATA.included_area_links==="object"?DATA.included_area_links:{};return String(links[String(label||"").trim()]||"").trim()}
function includedAreaHtml(label){const safe=String(label||"").trim();const id=includedAreaLinkId(safe);if(!id)return "<div>"+esc(safe)+"</div>";return "<div><a href=\\"/credits/region-pack-map?token="+currentToken+"&region_pack_id="+encodeURIComponent(id)+"&catalog=1\\">"+esc(safe)+"</a></div>"}
...
async function loadAsset(id){const safe=String(id||"").trim();if(assetCache.has(safe))return assetCache.get(safe);const res=await fetch("/maps/region-pack-map-asset?region_pack_id="+encodeURIComponent(safe)+"&v="+assetVersion,{cache:"force-cache"});if(!res.ok)throw new Error("map_asset_"+res.status);const asset=await res.json();assetCache.set(safe,asset);return asset}
function normaliseTile(tile,owned){const p=parseTileKey(tile&&tile.tile_key);const f=family(p);const ownedDs=owned.get(f)||new Set();const gross=int(tile&&((tile.gross_cents!==undefined?tile.gross_cents:tile.full_price_cents)));const globallyFree=!!(tile&&tile.globally_free)||gross<=0;let status="new";if(globallyFree){status="free"}else if(DATA.scene_detail&&String(tile&&tile.status||"").trim()){status=String(tile.status).trim()}else if(DATA.world_full_quality_unlocked||DATA.product_full_quality_unlocked||Array.from(ownedDs).some((d)=>Number(d)<=Number(p&&p.d||0))){status="licenced"}else if(Array.from(ownedDs).some((d)=>Number(d)>Number(p&&p.d||0))){status="partial"}if(DATA.scene_detail&&p&&Number(p.d)>Number(p.z)&&status!=="licenced"&&status!=="free"){status="lower_resolution"}const lonMin=Number(tile&&tile.lon_min),lonMax=Number(tile&&tile.lon_max),latMin=Number(tile&&tile.lat_min),latMax=Number(tile&&tile.lat_max);return{...tile,tile_key:p?p.key:String(tile&&tile.tile_key||""),x:p?p.x:null,y:p?p.y:null,z:p?p.z:null,d:p?p.d:null,lon_min:Number.isFinite(lonMin)?lonMin:(p?p.x-180:null),lon_max:Number.isFinite(lonMax)?lonMax:(p?p.x-180+p.z:null),lat_min:Number.isFinite(latMin)?latMin:(p?p.y-90:null),lat_max:Number.isFinite(latMax)?latMax:(p?p.y-90+p.z:null),full_price_cents:gross,gross_cents:gross,status}}
...
function renderCards(){const s=DATA.summary;if(!s){const label=DATA.price_pending?"Price updating":"Price unavailable";const detail=DATA.price_pending?"Please wait a few moments.":"Reopen this page from Blender or the data-pack list.";document.getElementById("cards").innerHTML='<div class="card final-price"><span>'+label+'</span><b>'+detail+'</b></div>';return}const partialTiles=Number(s.partial_licence_tiles||0);const alreadyTiles=Number(s.already_licenced_tiles||0)+partialTiles;const alreadyValue=int(s.already_licenced_deduction_cents)+int(s.partial_licence_credit_cents);const discountValue=int(s.discount_cents);const newTiles=Math.max(0,Number(s.new_tiles||0)-partialTiles);const cards=[["New Tiles / Total Tiles",fmtNum(newTiles)+" / "+fmtNum(s.total_tiles)]];if(!DATA.scene_detail)cards.push(["Data Size",String(s.data_size_text||DATA.region_pack&&DATA.region_pack.data_size_text||fmtDataSize(s.data_size_bytes||DATA.region_pack&&DATA.region_pack.data_size_bytes||0))]);cards.push(["Full Price",fmtCents(s.full_price_cents)]);if(alreadyValue>0)cards.push(["Already Licenced",fmtNum(alreadyTiles)+" tiles\\n-"+fmtCents(alreadyValue)]);if(discountValue>0)cards.push(["Volume Discount",Number(s.discount_percent||0)+"%\\n-"+fmtCents(discountValue)]);if(DATA.scene_detail&&int(s.custom_scene_licence_cents)>0)cards.push([s.scene_custom_licence_label||"Custom scene-specific licence",fmtCents(s.custom_scene_licence_cents)]);const buy=currentBuyHref()?'<a class="button buy-now" href="'+currentBuyHref()+'">'+(int(s.price_cents)>0?"Buy Now":"Licence Now")+'</a>':"";cards.push(["Final Price",fmtCents(s.price_cents),buy]);document.getElementById("cards").innerHTML=cards.map((c)=>'<div class="card '+(c[0]==="Final Price"?"final-price":"")+'"><span>'+esc(c[0])+'</span><b>'+esc(c[1])+'</b>'+(c[2]||"")+'</div>').join("")}
function frameForBounds(rawBounds,width,minHeight,maxHeight,padSize){const b=rawBounds||{min_lon:-10,min_lat:35,max_lon:30,max_lat:48};const minLon=Number.isFinite(Number(b.min_lon))?Number(b.min_lon):-10,minLat=Number.isFinite(Number(b.min_lat))?Number(b.min_lat):35,maxLon=Number.isFinite(Number(b.max_lon))?Number(b.max_lon):30,maxLat=Number.isFinite(Number(b.max_lat))?Number(b.max_lat):48;const fullWorld=minLon<=-179.999&&maxLon>=179.999&&minLat<=-89.999&&maxLat>=89.999;const pad=fullWorld?0:Math.max(0,Number(padSize)||0);const lonSpan=Math.max(1e-6,maxLon-minLon),latSpan=Math.max(1e-6,maxLat-minLat),innerW=Math.max(1,width-pad*2);const naturalH=Math.round(latSpan*(innerW/lonSpan))+pad*2,height=Math.max(minHeight,Math.min(maxHeight,naturalH)),innerH=Math.max(1,height-pad*2),scale=Math.min(innerW/lonSpan,innerH/latSpan),usedW=lonSpan*scale,usedH=latSpan*scale;return{bounds:{min_lon:minLon,min_lat:minLat,max_lon:maxLon,max_lat:maxLat},width,height,scale,ox:(width-usedW)/2,oy:(height-usedH)/2}}
...
async function loadProductOutlines(asset){const current=Array.isArray(asset&&asset.outlines)?asset.outlines:[];const merged=[];try{const response=await fetch(miniOutlinesUrl,{cache:"force-cache"});if(response.ok){const payload=await response.json();const outlines=Array.isArray(payload&&payload.outlines)?payload.outlines:[];merged.push(...outlines)}}catch(_error){}const region=asset&&asset.region_pack||{};if(region&&region.show_subdivision_outline){const id=region.id||currentPackId();try{const response=await fetch(outlinesUrl(id),{cache:"force-cache"});if(response.ok){const payload=await response.json();const outlines=Array.isArray(payload&&payload.outlines)?payload.outlines:[];merged.push(...outlines.map((outline)=>({...outline,subdivision_outline:true})))}}catch(_error){}}if(merged.length){asset.outlines=merged;return merged}return current}
function drawProductOutlines(svg,asset){const outlines=Array.isArray(asset&&asset.outlines)?asset.outlines:[];if(!svg||!outlines.length)return;const group=el("g",{class:"product-outlines","aria-hidden":"true"});for(const outline of outlines){for(const ring of Array.isArray(outline&&outline.polygons)?outline.polygons:[]){const path=drawOutlineRingPath(ring);if(!path)continue;const p=el("path",{d:path,fill:"none",stroke:outline&&outline.subdivision_outline?"#ffffff":"var(--country-line)","stroke-width":"0.7",opacity:outline&&outline.subdivision_outline?"0.9":"0.72","vector-effect":"non-scaling-stroke"});const t=el("title",{});t.textContent=outline.name||"";p.appendChild(t);group.appendChild(p)}}svg.appendChild(group)}
...
function mapStateShardUrl(level,index){const q=encodeURIComponent(String(DATA.quote&&DATA.quote.quote_id||""));return"/maps/region-pack-map-state-shard?token="+currentToken+"&region_pack_id="+currentPackIdEncoded+currentCatalog+"&quote_id="+q+"&level="+encodeURIComponent(String(level))+"&shard="+encodeURIComponent(String(index))}
function mapLevelChunkUrl(level,cursor){const q=encodeURIComponent(String(DATA.quote&&DATA.quote.quote_id||""));let url="/maps/region-pack-map-level-chunk?token="+currentToken+"&region_pack_id="+currentPackIdEncoded+currentCatalog+"&quote_id="+q+"&level="+encodeURIComponent(String(level));if(cursor&&cursor.family_key&&cursor.tile_key){url+="&cursor_family_key="+encodeURIComponent(cursor.family_key)+"&cursor_d="+encodeURIComponent(String(cursor.d||0))+"&cursor_tile_key="+encodeURIComponent(cursor.tile_key)}return url}
function mapCounts(){return{total:0,newCount:0,licencedCount:0,partialCount:0,freeCount:0}}
function setMapStatus(text,className){const node=document.getElementById("mapStatus");if(!node)return;if(className)node.className=className;node.textContent=text}
function queuedMapLoadingText(){return "The system is busy recalculating map tiles. Large maps may take a few minutes to become available."}
function normalMapLoadingText(){return DATA.map_pending&&!DATA.map_state_ready&&!DATA.scene_detail?queuedMapLoadingText():"Loading map..."}
function setMapLoading(active,text){const overlay=document.getElementById("mapLoadingOverlay");if(!overlay)return;overlay.classList.remove("failed");overlay.classList.toggle("hidden",!active);if(!overlay.querySelector("span"))overlay.innerHTML="<span></span>";const label=overlay.querySelector("span");if(label)label.textContent=text||normalMapLoadingText()}
function setMapFailure(message,retry){const text=message||"Map service is busy. Please try loading the map again in a few moments.";setMapStatus(text,"error small");const overlay=document.getElementById("mapLoadingOverlay");if(!overlay)return;overlay.classList.remove("hidden");overlay.classList.add("failed");overlay.innerHTML="<span><strong>Map service busy</strong><em>"+esc(text)+"</em><button class='map-retry' type='button'>Try again</button></span>";const button=overlay.querySelector("button");if(button&&typeof retry==="function")button.addEventListener("click",()=>{setMapLoading(true);window.setTimeout(()=>retry(),350)},{once:true})}
function completeMapLoading(){window.setTimeout(()=>setMapLoading(false),120)}
function updateMapSummary(level,counts,loaded,expected,complete){const summary=document.getElementById("levelSummary");if(complete){if(summary)summary.textContent=fmtNum(counts.total)+" tiles at "+zoomLabel(level)+" · new "+fmtNum(counts.newCount)+" · already licenced "+fmtNum(counts.licencedCount+counts.partialCount)+" · free "+fmtNum(counts.freeCount);completeMapLoading();return}if(summary)summary.textContent="Loading "+zoomLabel(level)+" tiles... "+fmtNum(loaded)+" / "+fmtNum(expected);setMapLoading(true)}
function safeDefaultMapLevel(vm,levels,preferred){const available=(Array.isArray(levels)?levels:[]).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);if(!available.length)return Number(preferred)||1;if(DATA.scene_detail&&available.includes(Number(preferred)))return Number(preferred);const productId=String(vm&&vm.asset&&vm.asset.region_pack&&vm.asset.region_pack.id||DATA.region_pack&&DATA.region_pack.id||"").trim().toLowerCase();if(productId==="world"&&available.includes(3))return 3;if(DATA.map_state_ready){if(WORLD_FRAME_ZOOM2_DEFAULT_IDS.has(productId)&&available.includes(2))return 2;if(available.includes(1))return 1;}if(available.includes(Number(preferred)))return Number(preferred);return available[0]}
function setActiveZoomButton(level){const buttons=document.querySelectorAll(".zoom-button[data-level]");buttons.forEach((button)=>button.classList.toggle("active",Number(button.dataset.level)===Number(level)))}
function renderZoomButtons(vm,levels,defaultLevel){const toolbar=document.getElementById("mapToolbar");const buttons=document.getElementById("levelButtons");if(!toolbar||!buttons)return;buttons.replaceChildren();if(DATA.scene_detail){toolbar.style.display="none";return}toolbar.style.display="";const safeLevels=(Array.isArray(levels)?levels:[]).map(Number).filter(Number.isFinite);for(const [index,level] of safeLevels.entries()){const button=document.createElement("button");button.type="button";button.className="zoom-button";button.dataset.level=String(level);button.textContent=String(index+1);button.title=zoomLabel(level);button.setAttribute("aria-label",zoomLabel(level));button.addEventListener("click",()=>{setActiveZoomButton(level);renderMap(vm,level)});buttons.appendChild(button)}setActiveZoomButton(defaultLevel)}
...
function appendTileRows(svg,level,sourceRows,counts){const rows=(Array.isArray(sourceRows)?sourceRows:[]).filter((row)=>Number(row.z)===Number(level));for(const tile of rows){const a=xy(tile.lon_min,tile.lat_max),b=xy(tile.lon_max,tile.lat_min);const contextOnly=tile.status==="context";const lower=tile.status==="lower_resolution";const cls=contextOnly?"transparent":(tile.status==="new"?"var(--new)":((tile.status==="partial"||lower)?"var(--partial)":(tile.status==="licenced"?"var(--licenced)":"var(--free)")));if(!contextOnly){if(tile.status==="new"||lower)counts.newCount++;else if(tile.status==="partial")counts.partialCount++;else if(tile.status==="licenced")counts.licencedCount++;else counts.freeCount++}counts.total++;const r=el("rect",{class:"map-tile",x:a[0],y:a[1],width:Math.max(1,b[0]-a[0]),height:Math.max(1,b[1]-a[1]),fill:cls,stroke:"#fff","stroke-width":contextOnly?"2":"0.45","vector-effect":contextOnly?"non-scaling-stroke":"none",opacity:contextOnly?"0.26":((tile.status==="new"||tile.status==="partial"||lower)?"0.58":"0.43"),"pointer-events":contextOnly?"all":"auto"});const statusText=contextOnly?"outside this scene":(lower?"new lower resolution tile":(tile.status==="partial"?"partially licenced":tile.status));attachTileTooltip(r,tileTooltipText(tile,statusText));svg.appendChild(r)}}
function drawMapRows(vm,level,sourceRows){const rows=(Array.isArray(sourceRows)?sourceRows:[]).filter((row)=>Number(row.z)===Number(level));setBounds(vm&&vm.asset&&vm.asset.bounds);const svg=drawMapBase(vm);const counts=mapCounts();appendTileRows(svg,level,rows,counts);updateMapSummary(level,counts,counts.total,counts.total,true)}
...
async function renderMap(vm,level){const seq=++mapRenderSeq;const key=shardCacheKey(level);const expected=Math.max(0,Number(vm&&vm.state&&vm.state.level_tile_counts&&vm.state.level_tile_counts[key]||0)||0);setMapStatus("","muted small");if(vm.onDemand){const cached=onDemandRowsIfLoaded(vm,level);if(cached){drawMapRows(vm,level,cached);return}const svg=drawMapBase(vm),counts=mapCounts();updateMapSummary(level,counts,0,expected,false);try{const rows=await loadOnDemandRows(vm,level,(chunk,loaded,total)=>{if(seq!==mapRenderSeq)return;appendTileRows(svg,level,chunk,counts);updateMapSummary(level,counts,loaded,total,false)});if(seq===mapRenderSeq)drawMapRows(vm,level,rows)}catch(error){if(seq!==mapRenderSeq)return;console.warn("Planetka map level failed",error);setMapFailure("Map service is busy. Please try loading the map again in a few moments.",()=>renderMap(vm,level));throw error}return}if(vm.sharded){const cached=shardedRowsIfLoaded(vm,level);if(cached){drawMapRows(vm,level,cached);return}const shards=vm&&vm.state&&vm.state.level_shards&&vm.state.level_shards[key]||[];const svg=drawMapBase(vm),counts=mapCounts();updateMapSummary(level,counts,0,expected||0,false);try{const rows=await loadShardedRows(vm,level,(chunk,loaded)=>{if(seq!==mapRenderSeq)return;appendTileRows(svg,level,chunk,counts);updateMapSummary(level,counts,loaded,expected||loaded,false)});if(seq===mapRenderSeq)drawMapRows(vm,level,rows)}catch(error){if(seq!==mapRenderSeq)return;console.warn("Planetka map shard failed",error);setMapFailure("Map service is busy. Please try loading the map again in a few moments.",()=>renderMap(vm,level));throw error}return}drawMapRows(vm,level,vm.rows)}
function miniFrame(bounds,id){const rawId=String(id||"").trim().toLowerCase();if(rawId==="world"){const scale=1000/360;return{bounds:{min_lon:-180,min_lat:-90,max_lon:180,max_lat:90},width:1000,height:1000,scale,ox:0,oy:(1000-(180*scale))/2,wrap_lon:false}}const source=MINI_FRAME_BY_ID[rawId]||(PACIFIC_MINI_FRAME_IDS.has(rawId)?{min_lon:80,min_lat:-60,max_lon:230,max_lat:30,wrap_lon:true}:(bounds||{min_lon:-10,min_lat:35,max_lon:30,max_lat:48}));const frame=frameForBounds(source,1000,320,820,20);frame.wrap_lon=!!(source&&source.wrap_lon);return frame}
function miniProjectLon(frame,lon){let value=Number(lon);if(frame&&frame.wrap_lon){while(value<frame.bounds.min_lon)value+=360;while(value>frame.bounds.max_lon)value-=360}return value}
function miniXY(frame,lon,lat){const s=Number.isFinite(Number(frame.scale))?Number(frame.scale):1;const xLon=miniProjectLon(frame,lon);return[frame.ox+(xLon-frame.bounds.min_lon)*s,frame.oy+(frame.bounds.max_lat-lat)*s]}
function miniRawXY(frame,lon,lat){const s=Number.isFinite(Number(frame.scale))?Number(frame.scale):1;return[frame.ox+(lon-frame.bounds.min_lon)*s,frame.oy+(frame.bounds.max_lat-lat)*s]}
function addMiniMapBackground(svg,frame,id){const width=frame.width,height=frame.height;svg.appendChild(el("rect",{x:0,y:0,width,height,fill:"#0d1118"}));const rawId=String(id||"").trim().toLowerCase();const productHref=productMapBgHref(id);if(productHref&&isWorldFrameMapId(rawId)){const worldW=360*frame.scale;for(const startLon of [-180,180]){const tl=miniRawXY(frame,startLon,90),br=miniRawXY(frame,startLon+360,-90);const img=el("image",{href:productHref,x:tl[0],y:tl[1],width:worldW,height:br[1]-tl[1],preserveAspectRatio:"none",opacity:"1.0"});img.addEventListener("error",()=>img.remove());svg.appendChild(img)}}else if(productHref){const img=el("image",{href:productHref,x:0,y:0,width,height,preserveAspectRatio:"none",opacity:"1.0"});img.addEventListener("error",()=>img.remove());svg.appendChild(img)}else{for(const startLon of [-180,180]){const tl=miniRawXY(frame,startLon,90),br=miniRawXY(frame,startLon+360,-90);svg.appendChild(el("image",{href:MAP_BG,x:tl[0],y:tl[1],width:360*frame.scale,height:br[1]-tl[1],preserveAspectRatio:"none",opacity:"1.0"}))}}}
const miniOutlinesCache=new Map();async function loadMiniOutlines(id){const key=String(id||"").trim();if(!key)return[];if(!miniOutlinesCache.has(key)){miniOutlinesCache.set(key,fetch(outlinesUrl(key),{cache:"force-cache"}).then((res)=>res.ok?res.json():null).then((payload)=>Array.isArray(payload&&payload.outlines)?payload.outlines:[]).catch(()=>[]))}return miniOutlinesCache.get(key)}
function drawMiniOutlines(svg,frame,asset){const outlines=Array.isArray(asset&&asset.outlines)?asset.outlines:[];if(!outlines.length)return;const group=el("g",{class:"product-outlines","aria-hidden":"true"});for(const outline of outlines){for(const ring of Array.isArray(outline&&outline.polygons)?outline.polygons:[]){let path="",previousLon=null,started=false;for(const point of Array.isArray(ring)?ring:[]){const lon=Number(point&&point[0]),lat=Number(point&&point[1]);if(!Number.isFinite(lon)||!Number.isFinite(lat)){started=false;previousLon=null;continue}if(previousLon!==null&&Math.abs(lon-previousLon)>180)started=false;const p=miniXY(frame,lon,lat);path+=(started?"L":"M")+p[0].toFixed(2)+" "+p[1].toFixed(2);started=true;previousLon=lon}if(path)group.appendChild(el("path",{d:path,fill:"none",stroke:outline&&outline.subdivision_outline?"#ffffff":"var(--country-line)","stroke-width":"0.7",opacity:outline&&outline.subdivision_outline?"0.9":"0.72","vector-effect":"non-scaling-stroke"}))}}svg.appendChild(group)}
function renderMiniMap(svg,vm){const asset=vm&&vm.asset||{};const b=asset.bounds||{min_lon:-10,min_lat:35,max_lon:30,max_lat:48};const bgId=asset&&asset.region_pack&&asset.region_pack.id||"";const first=vm&&vm.levels&&vm.levels.length?vm.levels[0]:null;const rows=(Array.isArray(vm&&vm.rows)?vm.rows:[]).filter((row)=>Number(row.z)===Number(first));const frame=miniFrame(b,bgId),w=frame.width,h=frame.height;svg.setAttribute("viewBox","0 0 "+w+" "+h);svg.style.aspectRatio="1 / 1";svg.setAttribute("preserveAspectRatio","xMidYMid slice");svg.replaceChildren();addMiniMapBackground(svg,frame,bgId);drawMiniOutlines(svg,frame,asset);for(const tile of rows){const a=miniXY(frame,tile.lon_min,tile.lat_max),c=miniXY(frame,tile.lon_max,tile.lat_min);const lower=tile.status==="lower_resolution";const cls=tile.status==="new"?"var(--new)":((tile.status==="partial"||lower)?"var(--partial)":(tile.status==="licenced"?"var(--licenced)":"var(--free)"));svg.appendChild(el("rect",{x:a[0],y:a[1],width:Math.max(1,Math.abs(c[0]-a[0])),height:Math.max(1,c[1]-a[1]),fill:cls,stroke:"#fff","stroke-width":"0.5",opacity:(tile.status==="new"||tile.status==="partial"||lower)?"0.58":"0.43"}))}}
async function loadMiniMap(id,q){const url="/maps/region-pack-mini-map?token="+currentToken+"&region_pack_id="+encodeURIComponent(String(id||""))+currentCatalog+(q?"&quote_id="+encodeURIComponent(String(q)):"");const res=await fetch(url,{cache:"no-store"});if(!res.ok)throw new Error("mini_map_"+res.status);return res.json()}
function upsellGroupLabel(pack){const type=String(pack&&pack.type||"").trim().toLowerCase();if(type==="world")return "World";if(type==="continent")return "Continents";if(type==="macro_region")return "Regions";if(type==="admin_region")return "States / Provinces";return "Countries"}
function upsellRowFromCard(card){const pack=card&&card.region_pack||{};const summary=card&&card.summary||null;const q=String(card&&card.quote_id||"");const ready=!!(summary&&q);const partialCount=Number((summary&&summary.partial_licence_tiles)!==undefined?(summary&&summary.partial_licence_tiles):((summary&&summary.partial_licence_tile_count)!==undefined?(summary&&summary.partial_licence_tile_count):0));const partialCents=int(summary&&summary.partial_licence_credit_cents);const alreadyCents=int(summary&&summary.already_licenced_deduction_cents)+partialCents;const newTiles=Math.max(0,Number(summary&&summary.new_tiles||0)-partialCount);return{id:String(pack.id||card&&card.asset_id||""),quote_id:q,quote_status:ready?"ready":"updating",price_pending:!ready,status_label:"Price updating",name:String(pack.name||"Data Pack"),group_label:upsellGroupLabel(pack),data_size_bytes:Number(summary&&summary.data_size_bytes||pack.data_size_bytes||0),data_size_text:String(summary&&summary.data_size_text||pack.data_size_text||fmtDataSize(summary&&summary.data_size_bytes||pack.data_size_bytes||0)),total_tiles:ready?Number(summary.total_tiles||0):null,new_tiles:ready?newTiles:null,already_licenced_tiles:ready?Number(summary.already_licenced_tiles||0)+partialCount:null,already_licenced_deduction_cents:ready?alreadyCents:null,discount_percent:ready?Number(summary.discount_percent||0):null,discount_cents:ready?int(summary.discount_cents):null,full_price_cents:ready?int(summary.full_price_cents):null,price_cents:ready?int(summary.price_cents):null}}
function upsellCatalogRowHtml(row){const id=encodeURIComponent(row.id||"");const quote=encodeURIComponent(row.quote_id||"");const ready=String(row.quote_status||"ready")==="ready"&&!row.price_pending&&quote;const quoteParam=quote?"&quote_id="+quote:"";const pending='<span class="pending">'+esc(row.status_label||"Price updating")+'</span>';const mapHref='/credits/region-pack-map?token='+currentToken+'&region_pack_id='+id+'&catalog=1'+quoteParam;const name=ready?'<a href="'+mapHref+'">'+esc(row.name||"Data Pack")+'</a>':'<b>'+esc(row.name||"Data Pack")+'</b>';const mapLink=ready?'<a class="button secondary" href="'+mapHref+'">Detailed Map</a>':'<span class="button secondary disabled">Detailed Map</span>';return '<tr>'+'<td>'+name+'<div class="muted small">'+esc(row.group_label||"")+'</div></td>'+'<td>'+(ready?fmtNum(row.new_tiles):pending)+'</td>'+'<td>'+esc(row.data_size_text||fmtDataSize(row.data_size_bytes))+'</td>'+'<td class="price">'+(ready?fmtCents(row.price_cents):pending)+'</td>'+'<td class="catalog-actions">'+mapLink+'</td>'+'<td class="catalog-actions">'+(ready?'<a class="button" href="/credits/region-pack-checkout?token='+currentToken+'&region_pack_id='+id+'&catalog=1'+quoteParam+'">Buy</a>':'<span class="button disabled">Buy</span>')+'</td>'+'</tr>'}
function renderUpsells(){const serverUpsells=Array.isArray(DATA.upsells)?DATA.upsells:[];const grid=document.getElementById("upsellGrid");if(!grid||!serverUpsells.length||grid.dataset.rendered==="1")return;grid.dataset.rendered="1";const rows=serverUpsells.map(upsellRowFromCard).filter((row)=>row.id);const colgroup='<colgroup><col style="width:36%"><col style="width:12%"><col style="width:16%"><col style="width:16%"><col style="width:12%"><col style="width:8%"></colgroup>';grid.innerHTML='<table>'+colgroup+'<thead><tr><th>Data Pack</th><th>New Tiles</th><th>Data Size</th><th>Final Price</th><th>Detailed Map</th><th>Buy</th></tr></thead><tbody>'+rows.map(upsellCatalogRowHtml).join("")+'</tbody></table>';document.getElementById("upsellsPanel").style.display=rows.length?"":"none"}
function serverMapAsset(){const state=DATA.map_state_ready&&DATA.map_state&&typeof DATA.map_state==="object"?DATA.map_state:null;if(state&&state.on_demand){return{region_pack:mergeRegionPack(DATA.region_pack,state.region_pack),bounds:state.bounds||{min_lon:-10,min_lat:35,max_lon:30,max_lat:48},outlines:Array.isArray(state.outlines)?state.outlines:[],included_countries:Array.isArray(state.included_countries)?state.included_countries:[],tiles:[]}}if(DATA.scene_detail&&Array.isArray(DATA.tiles)&&DATA.tiles.length){return{region_pack:DATA.context_region_pack||DATA.region_pack||{id:"scene",name:"This Scene"},bounds:DATA.bounds||DATA.tile_bounds||{min_lon:-10,min_lat:35,max_lon:30,max_lat:48},outlines:Array.isArray(DATA.outlines)?DATA.outlines:[],included_countries:Array.isArray(DATA.included_countries)?DATA.included_countries:[],tiles:DATA.tiles}}return null}
function renderInitialPageChrome(){const titlePrefix=String(DATA.title_prefix||DATA.success&&DATA.success.context_title_prefix||"").trim();const explicitTitle=String(DATA.page_title||"").trim();const pack=DATA.region_pack||{};const title=document.getElementById("pageTitle");if(title)title.textContent=explicitTitle||((titlePrefix?titlePrefix+": ":"")+(pack.name||"Data Pack")+" Full Quality Pack");renderCards()}
async function loadAndRenderMap(){setMapLoading(true);const asset=serverMapAsset()||await loadAsset(DATA.asset_id);const vm=computeViewModel(asset);setBounds(vm.asset.bounds);const countries=uniqueCountryNames(vm.asset.included_countries);if(countries.length){document.getElementById("countries").innerHTML=countries.map(includedAreaHtml).join("");document.getElementById("countriesPanel").style.display=""}let levels=vm.levels.length?vm.levels:[1];const preferred=Number(vm&&vm.state&&vm.state.default_level);if(DATA.scene_detail&&levels.includes(preferred))levels=[preferred];window.PLANETKA_MAP_ZOOM_LEVELS=levels;const defaultLevel=safeDefaultMapLevel(vm,levels,preferred);renderZoomButtons(vm,levels,defaultLevel);await loadProductOutlines(vm.asset);await renderMap(vm,defaultLevel);if(!DATA.map_state_ready&&!DATA.scene_detail){const summary=document.getElementById("levelSummary");if(summary)summary.textContent=DATA.price_pending?"Price and map are updating. Please wait a few moments.":"Map is updating. Please wait a few moments.";return}}
function retryMapLoad(){const id=String(DATA.asset_id||DATA.region_pack&&DATA.region_pack.id||"").trim();if(id)assetCache.delete(id);loadAndRenderMap().then(()=>renderUpsells()).catch((error)=>{console.warn("Planetka region-pack map failed",error);setMapFailure("Map service is busy. Please try loading this map again in a few moments.",retryMapLoad);renderUpsells()})}
function init(){renderInitialPageChrome();retryMapLoad()}
init();`;
...
const REGION_PACK_PAGE_ASSETS = new Map([
  ...
  ["checkout-frisco4k.jpg", { content_type: "image/jpeg", body: () => checkoutPlanetJpegBytes(), cache_control: "public, max-age=604800, immutable" }],
  ["checkout-planet.jpg", { content_type: "image/jpeg", body: () => checkoutPlanetJpegBytes(), cache_control: "public, max-age=3600" }],
]);

let checkoutPlanetJpegBytesCache = null;

function checkoutPlanetJpegBytes() {
  if (checkoutPlanetJpegBytesCache) {
    return checkoutPlanetJpegBytesCache;
  }
  const binary = atob(CHECKOUT_PLANET_JPEG_BASE64);
  const bytes = new Uint8Array(binary.length);
  for (let index = 0; index < binary.length; index += 1) {
    bytes[index] = binary.charCodeAt(index);
  }
  checkoutPlanetJpegBytesCache = bytes;
  return checkoutPlanetJpegBytesCache;
}
...
export function handleCreditRegionPackPageAsset(request, env, deps) {
  ...
  const body = typeof asset.body === "function" ? asset.body() : asset.body;
  const response = new Response(body, {
    status: 200,
    headers: {
      "Content-Type": asset.content_type,
      "Cache-Control": asset.cache_control || "public, max-age=86400",
      ...corsHeaders(env),
    },
  });
  timing.mark("asset");
...
function regionPackStaticMapPayload(product, token, account, ownedRows, options = {}) {
  ...
  const upsells = storedUpsells || (Array.isArray(options && options.upsells)
    ? options.upsells
      .map((entry) => {
        if (entry && entry.region_pack) {
          return {
            ...entry,
            region_pack: entry.region_pack,
            asset_id: String(entry.asset_id || entry.region_pack.id || ""),
        ...
        return {
          ...
          summary: upsellQuote && upsellQuote.summary ? upsellQuote.summary : null,
        };
      })
      .filter(Boolean)
    : []);
...
async function relevantSceneProductPackReadyQuoteEntries(db, contextProduct, userId, account, deps, options = {}) {
  ...
  const similarProducts = await relatedSimilarRegionProducts(db, contextProduct, deps, 0);
  const contextRank = regionProductRank(contextProduct);
  const contextCountrySet = regionProductCountryIdSet(contextProduct);
  const parentProducts = REGION_PRODUCTS
    .filter((candidate) => {
      const candidateId = String(candidate && candidate.id || "").trim();
      if (!candidateId || isHiddenRegionProduct(candidate)) {
        return false;
      }
      if (regionProductRank(candidate) <= contextRank) {
        return false;
      }
      const candidateSet = regionProductCountryIdSet(candidate);
      return isCountryIdSetSubset(contextCountrySet, candidateSet);
    })
    .sort((a, b) => (
  ...
  const candidates = [contextProduct, ...similarProducts, ...parentProducts];
  const seen = new Set();
...
function regionMapBounds(product, detail, tileRows) {
  ...
  const mapBounds = product && Array.isArray(product.map_bounds) ? product.map_bounds : null;
...
function buildCanonicalRegionPackSummary(product, estimate) {
  ...
  return {
    ...
    free_tiles: Math.max(0, Number.parseInt(estimate && estimate.free_tile_count || 0, 10) || 0),
...
async function regionProductIdsContainingAllTileFamilies(db, tileKeys, deps) {
  const familyKeys = Array.from(new Set(normalizeTileKeys(tileKeys)
  ...
  await ensureRegionPackTileEntryTable(db, deps);
  const matchedFamiliesByProduct = new Map();
  for (const chunk of fixedSizeChunks(familyKeys, SQL_VARIABLE_SAFE_CHUNK_SIZE)) {
    const rows = await deps.dbAll(
      db,
      `
        SELECT DISTINCT region_pack_id, family_key
        FROM region_pack_tile_entries
        WHERE catalog_version = ?
          AND family_key IN (${chunk.map(() => "?").join(",")})
      `,
    ...
    for (const row of rows || []) {
      const productId = String(row && row.region_pack_id || "").trim().toLowerCase();
      const familyKey = String(row && row.family_key || "").trim();
      if (!productId || !familyKey) {
        continue;
      }
      if (!matchedFamiliesByProduct.has(productId)) {
        matchedFamiliesByProduct.set(productId, new Set());
      }
      matchedFamiliesByProduct.get(productId).add(familyKey);
    }
  }
  const requiredCount = familyKeys.length;
  const matches = [];
  for (const [productId, families] of matchedFamiliesByProduct.entries()) {
    if (families && families.size === requiredCount) {
...
async function sceneSuccessCandidateProductsForTileKeys(db, tileKeys, deps) {
  ...
  const candidateIds = await regionProductIdsContainingAllTileFamilies(db, keys, deps);
  const candidateProducts = candidateIds
    .map((productId) => regionProductById(productId))
    .filter(Boolean);
  const matches = [];
...
function buildSceneFullQualityMapData(estimate, options = {}) {
  ...
  return {
    ...
    included_countries: contextCountries,
    context_region_pack: contextProduct ? regionProductPublicPayload(contextProduct) : null,
    ...
    title_prefix: String(options && options.titlePrefix || "").trim(),
    page_title: String(options && options.pageTitle || "").trim(),
...
function regionPackOfferPayload(product, quote) {
  ...
  return {
    ...
    data_size_bytes: summary.data_size_bytes,
    data_size_text: summary.data_size_text,
...
const USER_PRODUCT_QUOTE_JOB_LOCK_TTL_SECONDS = 180;
...
const USER_PRODUCT_QUOTE_JOB_STALE_RUNNING_SECONDS = 300;
...
const REGION_PACK_MAP_PREWARM_PRIMARY_PRODUCT_IDS = [
  "europe",
  "north_america",
  "south_america",
  "asia",
...
const REGION_PACK_MAP_PREWARM_LARGEST_COUNTRY_PRODUCT_IDS = [
  "united_states",
  "canada",
  "china",
  "brazil",
...
const REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_IDS = [
  ...REGION_PACK_MAP_PREWARM_PRIMARY_PRODUCT_IDS,
  "world",
  ...REGION_PACK_MAP_PREWARM_LARGEST_COUNTRY_PRODUCT_IDS,
];
const REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_ORDER = new Map(
  REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_IDS.map((id, index) => [id, index]),
);
const REGION_PACK_MAP_PREWARM_FINAL_PRICE_THRESHOLD_CENTS = 100000;
const REGION_PACK_MAP_PREWARM_JOB_ROUND = 20;
const REGION_PACK_MAP_PREWARM_WORLD_DEFAULT_LEVEL_PRIORITY = 0;
const REGION_PACK_MAP_PREWARM_FIXED_PRIORITY_BASE = 300;
const REGION_PACK_MAP_PREWARM_DEFAULT_ZOOM_PRIORITY = 520;
const REGION_PACK_MAP_PREWARM_HIGH_VALUE_PRIORITY = 620;
...
function fullyCoveredRegionPackProductIdsForPurchasedProduct(sourceProductId) {
  const safeSourceId = String(sourceProductId || "").trim().toLowerCase();
  if (!safeSourceId || !regionProductById(safeSourceId)) {
    return [];
  }
  ...
  for (const row of relationRows) {
    ...
    if (
      targetProductId
      && targetProductId !== safeSourceId
      && relationType === "parent_covers_target"
      && regionProductById(targetProductId)
    ) {
...
async function markUserProductQuotesStale(db, userId, productIds, deps, reason, timestamp) {
  ...
  for (const chunk of fixedSizeChunks(ids, SQL_VARIABLE_SAFE_CHUNK_SIZE)) {
    await deps.dbRun(
      db,
      `
        DELETE FROM pricing_quotes
        WHERE user_id = ?
          AND quote_type = 'region_pack'
          AND catalog_version = ?
          AND LOWER(subject_id) IN (${chunk.map(() => "?").join(",")})
      `,
      [safeUserId, REGION_PACK_CATALOG_VERSION, ...chunk],
    );
    const result = await deps.dbRun(
      db,
      `
...
async function enqueueUserProductMapPrewarmJobIfNeeded(db, userId, product, pricingVersion, entitlementVersion, deps, options = {}) {
  const safeUserId = String(userId || "").trim();
  const productId = normalizedRegionPackProductId(product);
  if (!safeUserId || !productId) {
    return false;
  }
  const defaultMapLevel = Number.isFinite(Number(options && options.defaultMapLevel))
    ? Number.parseInt(options.defaultMapLevel, 10)
    : preferredRegionPackMapDefaultLevel(product, await regionPackTileLevelCountsForProduct(db, product, deps));
  const prioritySummary = options && options.summary;
  const shouldPrewarm = regionProductFixedOrHighValueMapPrewarm(product, prioritySummary) || defaultMapLevel > 1;
  if (!shouldPrewarm) {
    return false;
  }
  let quoteRow = options && options.quoteRow && typeof options.quoteRow === "object" ? options.quoteRow : null;
  if (!quoteRow) {
    const quoteRows = await loadUserProductQuoteRows(db, safeUserId, [productId], deps, { includeMapState: true });
    quoteRow = quoteRows.get(productId) || null;
  }
  if (productQuoteStatus(quoteRow, pricingVersion, entitlementVersion) !== "ready") {
    return false;
  }
  const existingMapState = await parseUserProductMapStatePayload(quoteRow, options && options.env || {}, { allowNonReady: false });
  if (userProductMapStateAllLevelsReady(existingMapState)) {
    return false;
  }
  return enqueueUserProductQuoteJob(db, safeUserId, productId, pricingVersion, entitlementVersion, deps, {
    batchId: String(options && options.batchId || "") || null,
    ...
    triggerType: String(options && options.triggerType || "post_purchase_map_state_prewarm"),
    staleReason: String(options && options.staleReason || "map_state_prewarm_requested"),
    jobRound: Number.isFinite(Number(options && options.jobRound)) ? Number.parseInt(options.jobRound, 10) : REGION_PACK_MAP_PREWARM_JOB_ROUND,
    priority: Number.isFinite(Number(options && options.priority))
      ? Number.parseInt(options.priority, 10)
      : regionProductMapPrewarmPriority(product, prioritySummary, defaultMapLevel),
  });
}

async function enqueueReadyPriorityMapPrewarmJobs(db, userId, pricingVersion, entitlementVersion, deps, options = {}) {
  const safeUserId = String(userId || "").trim();
  if (!safeUserId) {
    return { queued_map_job_count: 0, considered_product_count: 0 };
  }
  const candidates = [];
  const seen = new Set();
  for (const fixedProductId of REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_IDS) {
    const product = regionProductById(fixedProductId);
    if (product && !seen.has(fixedProductId)) {
      seen.add(fixedProductId);
      candidates.push(product);
    }
  }
  const defaultLevelByProductId = await productIdsWithDefaultMapLevelAboveOne(db, deps);
  for (const productId of defaultLevelByProductId.keys()) {
    const product = regionProductById(productId);
    if (product && !seen.has(productId) && !isHiddenRegionProduct(product)) {
      seen.add(productId);
      candidates.push(product);
    }
  }
  ...
  const productIds = candidates.map((product) => normalizedRegionPackProductId(product)).filter(Boolean);
  const quoteRows = await loadUserProductQuoteRows(db, safeUserId, productIds, deps, { includeMapState: true });
  let queued = 0;
  for (const product of candidates) {
    ...
    const summary = userProductQuoteSummaryFromRow(quoteRow);
    const defaultMapLevel = defaultLevelByProductId.get(productId) || 0;
    if (!regionProductFixedOrHighValueMapPrewarm(product, summary) && defaultMapLevel <= 1) {
      continue;
    }
    const inserted = await enqueueUserProductMapPrewarmJobIfNeeded(
      db,
      safeUserId,
      product,
      pricingVersion,
      ...
      deps,
      {
        ...
        summary,
        defaultMapLevel,
        jobRound: REGION_PACK_MAP_PREWARM_JOB_ROUND,
        priority: regionProductMapPrewarmPriority(product, summary, defaultMapLevel),
      },
    );
    if (inserted) {
      queued += 1;
  ...
  return { queued_map_job_count: queued, considered_product_count: candidates.length };
}
...
async function invalidateAndQueueUserProductQuotes(db, userId, deps, options = {}) {
  ...
  const tileKeys = normalizeTileKeys(
    options && (options.tileKeys || options.insertedTileKeys || options.deltaTileKeys) || [],
  );
  ...
  const prewarmResult = await enqueueReadyPriorityMapPrewarmJobs(db, safeUserId, pricingVersion, entitlementVersion, deps, {
    sourceProductId: sourceProductId || null,
    triggerPurchaseId: String(options && options.triggerPurchaseId || "") || null,
    triggerType: "post_purchase_map_state_prewarm",
    staleReason: "post_purchase_priority_map_state_prewarm",
    env: options && options.env || {},
  });
  return {
    ...result,
    queued_map_prewarm_job_count: Math.max(0, Number.parseInt(prewarmResult && prewarmResult.queued_map_job_count || 0, 10) || 0),
...
async function materializePurchasedRegionPackQuoteNow(db, userId, product, deps, purchasedQuote = null) {
  ...
  const quoteProductId = String(purchasedQuote && (purchasedQuote.subject_id || purchasedQuote.region_pack_id) || "").trim().toLowerCase();
  const productSummary = regionProductPricingSummary(product) || {};
  const totalTiles = quoteProductId === productId
    ? Math.max(0, Number.parseInt(sourceSummary.total_tiles || 0, 10) || 0)
    : Math.max(0, Number.parseInt(productSummary.tile_count || product && product.tile_count || 0, 10) || 0);
  const fullPriceCents = quoteProductId === productId
    ? integerCents(sourceSummary.full_price_cents)
    : Math.max(0, Number.parseInt(product && product.gross_cents || productSummary.gross_cents || 0, 10) || 0);
  const freeTiles = Math.max(0, Math.min(
    totalTiles,
    Number.parseInt(productSummary.free_tile_count || product && product.free_tile_count || 0, 10) || 0,
  ));
...
async function refreshUserProductQuoteJobLock(db, deps, lockToken) {
  const safeToken = String(lockToken || "").trim();
  if (!safeToken) {
    return;
  }
  ...
  await deps.dbRun(
    ...
    `
      ...
      WHERE lock_name = ?
        AND lock_token = ?
    `,
    [addSecondsIsoFromDeps(deps, USER_PRODUCT_QUOTE_JOB_LOCK_TTL_SECONDS), now, USER_PRODUCT_QUOTE_JOB_LOCK_NAME, safeToken],
  );
}
...
function userProductQuoteJobQueueMode(options = {}) {
  const raw = String(
    options && (options.jobMode || options.queueMode || options.jobKind) || "any",
  ).trim().toLowerCase();
  if (raw === "quote" || raw === "quotes") {
    return "quote";
  }
  if (raw === "map" || raw === "maps" || raw === "map_state" || raw === "map-state") {
    return "map";
  }
  return "any";
}

function userProductQuoteJobMapSqlPredicate() {
  return `(
...
async function claimNextUserProductQuoteJob(db, deps, lockToken, workerId, options = {}) {
  const now = quoteIsoNow(deps);
  const queueMode = userProductQuoteJobQueueMode(options);
  const mapPredicate = userProductQuoteJobMapSqlPredicate();
  const modePredicate = queueMode === "map"
    ? `AND ${mapPredicate}`
    : queueMode === "quote"
      ? `AND NOT ${mapPredicate}`
      : "";
  const row = await deps.dbGet(
    db,
    `
...
async function storeUserProductMapStateManifest(db, quote, mapState, deps, options = {}) {
  ...
  const mapStateJson = await userProductMapStateStoragePayload(options && options.env, quote, mapState);
  await deps.dbRun(
    db,
    `
      UPDATE user_product_quotes
...
function userProductMapStateAllLevelsReady(mapState) {
  if (!mapState || typeof mapState !== "object") {
    return false;
  }
  if (mapState.all_levels_ready === true) {
    return true;
  }
  ...
  if (mapState.sharded) {
    return !shardedMapStateNextPendingLevel(mapState);
  }
  const tiles = Array.isArray(mapState.tiles) ? mapState.tiles : [];
  if (!tiles.length) {
    return false;
  }
  const presentLevels = new Set(tiles.map((tile) => Number(tile && tile.z)).filter(isRegionPackPaidZLevel));
  return levels.every((level) => presentLevels.has(level));
}

function regionProductDefaultMapLevel(product) {
  const counts = product && product.counts_by_z && typeof product.counts_by_z === "object"
    ? product.counts_by_z
    : {};
  return preferredRegionPackMapDefaultLevel(product, counts);
}

function regionProductFixedOrHighValueMapPrewarm(product, summary = null) {
  const productId = normalizedRegionPackProductId(product);
  if (!productId || !product || isHiddenRegionProduct(product)) {
    return false;
  }
  if (REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_ORDER.has(productId)) {
    return true;
  }
  return integerCents(summary && summary.price_cents) > REGION_PACK_MAP_PREWARM_FINAL_PRICE_THRESHOLD_CENTS;
}

async function regionProductNeedsPriorityMapPrewarm(db, product, summary, deps) {
  if (regionProductFixedOrHighValueMapPrewarm(product, summary)) {
    return true;
  }
  const counts = await regionPackTileLevelCountsForProduct(db, product, deps);
  return preferredRegionPackMapDefaultLevel(product, counts) > 1;
}

function regionProductMapPrewarmPriority(product, summary = null, defaultLevel = 0, options = {}) {
  const productId = normalizedRegionPackProductId(product);
  const level = Math.max(0, Number.parseInt(defaultLevel || regionProductDefaultMapLevel(product) || 0, 10) || 0);
  if (
    productId === "world"
    && level === 3
    && !Boolean(options && options.afterDefaultReady)
  ) {
    return REGION_PACK_MAP_PREWARM_WORLD_DEFAULT_LEVEL_PRIORITY;
  }
  if (REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_ORDER.has(productId)) {
    return REGION_PACK_MAP_PREWARM_FIXED_PRIORITY_BASE + REGION_PACK_MAP_PREWARM_FIXED_PRODUCT_ORDER.get(productId);
  }
  if (level > 1) {
    return REGION_PACK_MAP_PREWARM_DEFAULT_ZOOM_PRIORITY + Math.min(99, level);
  }
  if (integerCents(summary && summary.price_cents) > REGION_PACK_MAP_PREWARM_FINAL_PRICE_THRESHOLD_CENTS) {
    return REGION_PACK_MAP_PREWARM_HIGH_VALUE_PRIORITY;
  }
  return REGION_PACK_MAP_PREWARM_HIGH_VALUE_PRIORITY + 50;
}

async function productIdsWithDefaultMapLevelAboveOne(db, deps) {
  await ensureRegionPackTileEntryTable(db, deps);
  const rows = await deps.dbAll(
    db,
    `
      SELECT region_pack_id, z, COUNT(*) AS count
      FROM region_pack_tile_entries
      WHERE catalog_version = ?
        AND d = z
      GROUP BY region_pack_id, z
      ORDER BY region_pack_id ASC, z ASC
    `,
    [REGION_PACK_CATALOG_VERSION],
  );
  ...
  for (const row of rows || []) {
    const productId = String(row && row.region_pack_id || "").trim().toLowerCase();
    const z = Number.parseInt(row && row.z || 0, 10);
    if (!productId || !isRegionPackPaidZLevel(z)) {
      continue;
    }
    if (!countsByProduct.has(productId)) {
      countsByProduct.set(productId, {});
    }
    countsByProduct.get(productId)[String(z)] = Math.max(0, Number.parseInt(row && row.count || 0, 10) || 0);
  }
  const result = new Map();
  for (const [productId, counts] of countsByProduct.entries()) {
    const defaultLevel = preferredRegionPackMapDefaultLevel(productId, counts);
    if (defaultLevel > 1) {
      result.set(productId, defaultLevel);
    }
  }
  return result;
}
...
async function materializeUserProductQuoteFromDeltaJob(db, job, product, account, deps) {
  // Pricing must be derived from the complete current entitlement state. The
  // retired delta shortcut updated stale quotes from incomplete baselines after
  // multiple purchases, which could make page prices disagree with map tiles.
  return null;
}
...
async function processSingleUserProductQuoteJob(db, job, deps, runContext = {}) {
  ...
  if (!quote) {
    ...
    quote = await createRegionPackQuote(db, userId, product, deps, {
      account,
  ...
  if (jobIncludesMapState) {
    let mapResult = null;
    if (userProductMapStateShouldShard(product)) {
      mapResult = await buildUserProductShardedMapStateStep(db, product, quote, account, deps, {
        env: runContext && runContext.env || {},
        requireAllLevelsReady: true,
      });
    } else {
      const mapState = await buildUserProductMapState(db, product, quote, account, deps, {
        env: runContext && runContext.env || {},
      });
      const levels = Array.isArray(mapState && mapState.levels) ? mapState.levels : [];
      mapState.all_levels_ready = true;
      mapState.default_level = preferredReadyRegionPackMapDefaultLevel(product, levels, mapState && mapState.level_tile_counts);
      await storeUserProductMapStateManifest(db, quote, mapState, deps, { status: "ready", env: runContext && runContext.env || {} });
      mapResult = {
        complete: true,
        processed: Math.max(0, Number.parseInt(mapState && mapState.tile_status_count || 0, 10) || 0),
        expected: Math.max(0, Number.parseInt(mapState && mapState.total_tile_status_count || 0, 10) || 0),
        default_ready: true,
      };
    }
    await finishUserProductQuoteJob(db, job, deps, "finished");
    if (mapResult && !mapResult.complete) {
      await enqueueUserProductQuoteJob(db, userId, productId, pricingVersion, entitlementVersion, deps, {
        ...
        triggerType: "map_state_shard_prewarm_step",
        staleReason: "map_state_shard_prewarm_pending",
        jobRound: Math.max(REGION_PACK_MAP_PREWARM_JOB_ROUND, Number.parseInt(job && job.job_round || 0, 10) || 0),
        priority: Math.max(
          regionProductMapPrewarmPriority(product, summary, 0, { afterDefaultReady: Boolean(mapResult && mapResult.default_ready) }),
          Number.parseInt(job && job.priority || 0, 10) || 0,
        ),
      });
    }
    return {
      status: mapResult && mapResult.complete ? "map_state_ready" : "map_state_progress",
      product_id: productId,
      quote_id: String(quote.quote_id || ""),
      final_price_cents: integerCents(summary && summary.price_cents),
      processed: Math.max(0, Number.parseInt(mapResult && mapResult.processed || 0, 10) || 0),
      expected: Math.max(0, Number.parseInt(mapResult && mapResult.expected || 0, 10) || 0),
      default_ready: Boolean(mapResult && mapResult.default_ready),
    };
  }
  await finishUserProductQuoteJob(db, job, deps, "finished");
  await enqueueUserProductMapPrewarmJobIfNeeded(db, userId, product, pricingVersion, entitlementVersion, deps, {
    env: runContext && runContext.env || {},
    summary,
    sourceProductId: String(job && job.source_product_id || "") || null,
    triggerPurchaseId: String(job && job.trigger_purchase_id || "") || null,
    triggerType: "post_quote_map_state_prewarm",
    staleReason: "post_quote_priority_map_state_prewarm",
    jobRound: REGION_PACK_MAP_PREWARM_JOB_ROUND,
    priority: regionProductMapPrewarmPriority(product, summary),
  });
  return {
    status: "finished",
    product_id: productId,
...
export async function processUserProductQuoteJobs(db, env = {}, deps = {}, options = {}) {
  ...
  const hasOption = (name) => Object.prototype.hasOwnProperty.call(Object(options || {}), name);
  const rawMaxJobs = hasOption("maxJobs") ? options.maxJobs : USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_JOBS;
  const rawMaxMs = hasOption("maxMs") ? options.maxMs : USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_MS;
  const rawMaxMapJobs = hasOption("maxMapJobs") ? options.maxMapJobs : USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_MAP_JOBS;
  const rawMaxHeavyMapJobs = hasOption("maxHeavyMapJobs") ? options.maxHeavyMapJobs : USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_HEAVY_MAP_JOBS;
  const maxJobs = Math.max(1, Math.min(60, Number.parseInt(rawMaxJobs, 10) || USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_JOBS));
  const maxMs = Math.max(500, Math.min(28000, Number.parseInt(rawMaxMs, 10) || USER_PRODUCT_QUOTE_JOB_DEFAULT_MAX_MS));
  const maxMapJobs = Math.max(0, Math.min(30, Number.parseInt(rawMaxMapJobs, 10) || 0));
  const maxHeavyMapJobs = Math.max(0, Math.min(maxMapJobs, Number.parseInt(rawMaxHeavyMapJobs, 10) || 0));
  const jobMode = userProductQuoteJobQueueMode(options);
  ...
  try {
    ...
    while (processed < maxJobs && (monotonicNowMs() - startedMs) < maxMs) {
      const job = await claimNextUserProductQuoteJob(db, deps, lockToken, workerId, { jobMode });
      if (!job) {
        break;
      }
      await refreshUserProductQuoteJobLock(db, deps, lockToken);
      const jobIncludesMapState = userProductQuoteJobWantsMapState(job);
...
function userProductQuoteSummaryFromRow(row) {
  ...
  return {
    ...
    data_size_bytes: dataSizeBytesForProduct(regionProductById(row.product_id)),
    data_size_text: formatDataSize(dataSizeBytesForProduct(regionProductById(row.product_id))),
...
function assertMapStateMatchesQuoteSummary(productId, mapTiles, quote) {
  const summary = quote && quote.summary && typeof quote.summary === "object" ? quote.summary : {};
  const fullCents = (Array.isArray(mapTiles) ? mapTiles : [])
    .reduce((total, tile) => total + Math.max(0, integerCents(tile && tile.full_price_cents)), 0);
  const alreadyCents = (Array.isArray(mapTiles) ? mapTiles : [])
    .reduce((total, tile) => total + Math.max(0, integerCents(tile && tile.already_licenced_cents)), 0);
  const partialCents = (Array.isArray(mapTiles) ? mapTiles : [])
    .reduce((total, tile) => total + Math.max(0, integerCents(tile && tile.partial_licence_credit_cents)), 0);
  const preDiscountCents = (Array.isArray(mapTiles) ? mapTiles : [])
    .reduce((total, tile) => total + Math.max(0, integerCents(tile && tile.pre_discount_cents)), 0);
  const expectedFullCents = integerCents(summary.full_price_cents);
  const expectedAlreadyCents = integerCents(summary.already_licenced_deduction_cents);
  const expectedPartialCents = integerCents(summary.partial_licence_credit_cents);
  const expectedPreDiscountCents = Math.max(0, expectedFullCents - expectedAlreadyCents - expectedPartialCents);
  if (fullCents !== expectedFullCents) {
    throw new Error(`map_state_full_price_mismatch:${productId}:${fullCents}:${expectedFullCents}`);
  }
  if (alreadyCents !== expectedAlreadyCents) {
    throw new Error(`map_state_already_licenced_mismatch:${productId}:${alreadyCents}:${expectedAlreadyCents}`);
  }
  if (partialCents !== expectedPartialCents) {
    throw new Error(`map_state_partial_licence_mismatch:${productId}:${partialCents}:${expectedPartialCents}`);
  }
  if (preDiscountCents !== expectedPreDiscountCents) {
    throw new Error(`map_state_pre_discount_mismatch:${productId}:${preDiscountCents}:${expectedPreDiscountCents}`);
  }
}
...
async function buildUserProductMapState(db, product, quote, account, deps, options = {}) {
  ...
  return {
    ...
    included_countries: regionProductIncludedCountries(product),
    levels,
    default_level: preferredReadyRegionPackMapDefaultLevel(product, levels, levelCounts),
    all_levels_ready: true,
    tiles: mapTiles,
  };
}
...
function regionPackMapTileCountFromLevelCounts(levelCounts) {
  if (!levelCounts || typeof levelCounts !== "object") {
    return 0;
  }
  return Object.entries(levelCounts).reduce((total, [level, count]) => {
    const z = Number.parseInt(level || 0, 10);
    if (!isRegionPackPaidZLevel(z)) {
      return total;
    }
    return total + Math.max(0, Number.parseInt(count || 0, 10) || 0);
  }, 0);
}
...
async function initialUserProductShardedMapState(db, product, quote, context, deps) {
  ...
  const levels = Object.keys(levelCounts)
    ...
    .filter((level) => isRegionPackPaidZLevel(level))
    .sort((a, b) => a - b);
  const defaultLevel = preferredRegionPackMapDefaultLevel(product, levelCounts);
  ...
  return {
    ...
    tile_status_count: 0,
    total_tile_status_count: regionPackMapTileCountFromLevelCounts(levelCounts),
    product_full_quality_unlocked: Boolean(context && context.targetFullyCovered),
    generated_at: quoteIsoNow(deps),
    catalog_version: REGION_PACK_CATALOG_VERSION,
    pricing_version: String(quote && quote.pricing_version || ""),
...
async function buildUserProductShardedMapStateStep(db, product, quote, account, deps, options = {}) {
  ...
  const context = await buildUserProductMapStateContext(db, product, quote, account, deps, options);
  const expectedTileCount = regionPackMapTileCountFromLevelCounts(await regionPackTileLevelCountsForProduct(db, product, deps));
  ...
  let activeLevel = Math.max(0, Number.parseInt(progress.active_level || 0, 10) || 0);
  ...
  const activeBuilt = activeLevel ? shardedMapStateLevelBuiltCount(mapState, activeLevel) : 0;
  if (
    !activeLevel
    || !activeExpected
    || activeBuilt >= activeExpected
  ...
  if (!activeLevel || !activeExpected) {
    ...
    mapState.all_levels_ready = true;
    mapState.default_level = preferredReadyRegionPackMapDefaultLevel(product, mapState.levels, mapState.level_tile_counts);
    await storeUserProductMapStateManifest(db, quote, mapState, deps, { status: "ready", env });
  ...
  const builtForActiveLevel = shardedMapStateLevelBuiltCount(mapState, activeLevel);
  if (builtForActiveLevel >= activeExpected) {
    ...
    mapState.all_levels_ready = Boolean(allComplete);
    if (allComplete) {
      mapState.default_level = preferredReadyRegionPackMapDefaultLevel(product, levels, levelCounts);
    }
    const requireAllLevelsReady = Boolean(options && options.requireAllLevelsReady);
    await storeUserProductMapStateManifest(db, quote, mapState, deps, { status: (requireAllLevelsReady ? allComplete : defaultReady) ? "ready" : "building", env });
  ...
  mapState.all_levels_ready = Boolean(complete);
  if (complete) {
    mapState.default_level = preferredReadyRegionPackMapDefaultLevel(product, levels, levelCounts);
  }
  const requireAllLevelsReady = Boolean(options && options.requireAllLevelsReady);
  await storeUserProductMapStateManifest(db, quote, mapState, deps, {
    status: (requireAllLevelsReady ? complete : defaultReady) ? "ready" : "building",
    env,
  });
...
function regionPackCheckoutQuoteIsCurrent(quote, userId, product, account) {
  const productId = String(product && product.id || "").trim().toLowerCase();
  const quoteProductId = String(quote && (quote.subject_id || quote.region_pack_id) || "").trim().toLowerCase();
  return Boolean(
    quote
...
function regionPackMapHtml(data) {
  ...
  const productLicenceNotes = `
<section class="panel product-licence-notes">
<h2>Licensing Information</h2>
<p>Licenced tiles remain available to your Planetka account permanently.</p>
<p>Licenced tiles can be downloaded through the Planetka add-on for local/offline use.</p>
<p>Already licenced tiles are automatically deducted from future Full Quality purchases.</p>
<div class="product-licence-links">
<a href="https://api.planetka.io/legal/terms-of-service.pdf" target="_blank" rel="noopener noreferrer">Terms and Conditions</a>
<a href="https://api.planetka.io/legal/privacy-policy.pdf" target="_blank" rel="noopener noreferrer">Privacy Policy</a>
<a href="https://api.planetka.io/legal/using-planetka-commercially.txt" target="_blank" rel="noopener noreferrer">Using Planetka Commercially</a>
<a href="https://api.planetka.io/legal/attribution-for-user-renders.txt" target="_blank" rel="noopener noreferrer">Attribution for User Renders</a>
</div>
</section>`;
  const includedAreaLinks = data && data.included_area_links && typeof data.included_area_links === "object" ? data.included_area_links : {};
  const includedAreaHtml = (country) => {
    const label = String(country || "").trim();
    const targetId = String(includedAreaLinks[label] || "").trim();
    if (!targetId) {
      return `<div>${escapeHtmlText(label)}</div>`;
    }
    const href = `/credits/region-pack-map?token=${tokenParam}&region_pack_id=${escapeHtmlText(encodeURIComponent(targetId))}&catalog=1`;
    return `<div><a href="${href}">${escapeHtmlText(label)}</a></div>`;
  };
  return `<!doctype html>
<html lang="en">
<head>
...
<style>${REGION_PACK_SHARED_CSS}</style>
</head>
<body>
<main>
<div class="page-heading">
<a class="catalog-back" href="/credits/region-pack-catalog?token=${tokenParam}">&larr; Product Catalog</a>
<h1>${isSceneDetail ? "Full Quality for This Scene" : `${escapeHtmlText(name)} Full Quality Pack`}</h1>
	...
	${success ? `<section class="panel"><h2>${escapeHtmlText(success.title || "Payment successful")}</h2><p>${escapeHtmlText(success.message || "Your Planetka purchase has been processed.")}</p></section>` : ""}
		<section class="cards">
		<div class="card"><span>New Tiles / Total Tiles</span><b>${formatInteger(newTiles)} / ${formatInteger(totalTiles)}</b></div>
		${!isSceneDetail ? `<div class="card"><span>Data Size</span><b>${escapeHtmlText(formatDataSize(summary.data_size_bytes ?? pack.data_size_bytes))}</b></div>` : ""}
		<div class="card"><span>Full Price</span><b>${formatEur(summary.full_price_eur)}</b></div>
		${alreadyLicencedDeductionEur > 0 ? `<div class="card"><span>Already Licenced</span><b>${formatInteger(alreadyLicencedTiles)} tiles\n-${formatEur(alreadyLicencedDeductionEur)}</b></div>` : ""}
		${Number(summary.discount_eur || 0) > 0 ? `<div class="card"><span>Volume Discount</span><b>${Number(summary.discount_percent || 0)}%\n-${formatEur(summary.discount_eur)}</b></div>` : ""}
		${customSceneLicenceEur > 0 ? `<div class="card"><span>${escapeHtmlText(summary.scene_custom_licence_label || SCENE_CUSTOM_LICENCE_LABEL)}</span><b>${formatEur(customSceneLicenceEur)}</b></div>` : ""}
		${customSceneLicenceEur <= 0 && sceneSmallFreeApplied ? `<div class="card"><span>Small Scene</span><b>Free below ${formatEur(summary.scene_small_free_threshold_eur || 0.5)}</b></div>` : ""}
	<div class="card final-price"><span>Final Price</span><b>${formatEur(summary.price_eur)}</b>${primaryBuyHref ? `<a class="button buy-now" href="${primaryBuyHref}">${Number(summary.price_eur || 0) > 0 ? "Buy Now" : "Licence Now"}</a>` : ""}</div>
	</section>
<section class="panel">
<div class="map-shell">
<div id="mapToolbar" class="map-toolbar-overlay">
<span class="map-toolbar-label">Zoom level</span>
<div id="levelButtons" class="zoom-buttons" role="group" aria-label="Zoom level"></div>
<span id="levelSummary" class="map-summary"></span>
</div>
...
<div id="mapLoadingOverlay" class="map-loading-overlay"><span>Loading map...</span></div>
	...
	<p class="muted small">Tile hover shows tile status and land coverage. The purchase price above is generated by the backend quote. Hover over any tile to see individual tile details.</p>
<div class="legend">
...
<div class="countries">${countries.map(includedAreaHtml).join("")}</div>
...
<h2>${isSceneDetail ? "Relevant Product Packs" : "Other Options"}</h2>
<div id="upsellGrid" class="upsells"></div>
</section>` : ""}
${isSceneDetail ? "" : productLicenceNotes}
</main>
...
function regionPackStaticSummaryCents(summary, centsKey, eurKey) {
  if (summary && summary[centsKey] !== undefined && summary[centsKey] !== null) {
    return integerCents(summary[centsKey]);
  }
  return integerCents(Math.round(Number(summary && summary[eurKey] || 0) * MONEY_SCALE));
}

function regionPackStaticCardsHtml(data) {
  const summary = data && data.summary || null;
  const pack = data && data.region_pack || {};
  if (!summary) {
    const label = data && data.price_pending ? "Price updating" : "Price unavailable";
    const detail = data && data.price_pending ? "Please wait a few moments." : "Reopen this page from Blender or the data-pack list.";
    return `<div class="card final-price"><span>${escapeHtmlText(label)}</span><b>${escapeHtmlText(detail)}</b></div>`;
  }
  const isSceneDetail = Boolean(data && data.scene_detail);
  const partialTiles = Math.max(0, Number.parseInt(summary.partial_licence_tiles ?? summary.partial_licence_tile_count ?? 0, 10) || 0);
  const alreadyTiles = Math.max(0, Number.parseInt(summary.already_licenced_tiles || 0, 10) || 0) + partialTiles;
  const alreadyValueCents = regionPackStaticSummaryCents(summary, "already_licenced_deduction_cents", "already_licenced_deduction_eur")
    + regionPackStaticSummaryCents(summary, "partial_licence_credit_cents", "partial_licence_credit_eur");
  const discountCents = regionPackStaticSummaryCents(summary, "discount_cents", "discount_eur");
  const customSceneLicenceCents = regionPackStaticSummaryCents(summary, "custom_scene_licence_cents", "custom_scene_licence_eur");
  const newTiles = Math.max(0, Number(summary.new_tiles || 0) - partialTiles);
  const totalTiles = Math.max(0, Number(summary.total_tiles || 0));
  const tokenParam = escapeHtmlText(encodeURIComponent(String(data && data.token || "")));
  const packIdParam = escapeHtmlText(encodeURIComponent(String(pack && pack.id || "")));
  const catalogParam = data && data.catalog_mode ? "&catalog=1" : "";
  const quoteParam = data && data.quote && data.quote.quote_id
    ? `&quote_id=${escapeHtmlText(encodeURIComponent(String(data.quote.quote_id || "")))}`
    : "";
  const buyHref = isSceneDetail && data && data.scene_checkout_url
    ? String(data.scene_checkout_url)
    : (!isSceneDetail && packIdParam ? `/credits/region-pack-checkout?token=${tokenParam}&region_pack_id=${packIdParam}${catalogParam}${quoteParam}` : "");
  const finalPriceCents = regionPackStaticSummaryCents(summary, "price_cents", "price_eur");
  const cards = [
    `<div class="card"><span>New Tiles / Total Tiles</span><b>${formatInteger(newTiles)} / ${formatInteger(totalTiles)}</b></div>`,
  ];
  if (!isSceneDetail) {
    cards.push(`<div class="card"><span>Data Size</span><b>${escapeHtmlText(summary.data_size_text || pack.data_size_text || formatDataSize(summary.data_size_bytes ?? pack.data_size_bytes))}</b></div>`);
  }
  cards.push(`<div class="card"><span>Full Price</span><b>${formatEur(centsToEur(regionPackStaticSummaryCents(summary, "full_price_cents", "full_price_eur")))}</b></div>`);
  if (alreadyValueCents > 0) {
    cards.push(`<div class="card"><span>Already Licenced</span><b>${formatInteger(alreadyTiles)} tiles\n-${formatEur(centsToEur(alreadyValueCents))}</b></div>`);
  }
  if (discountCents > 0) {
    cards.push(`<div class="card"><span>Volume Discount</span><b>${Number(summary.discount_percent || 0)}%\n-${formatEur(centsToEur(discountCents))}</b></div>`);
  }
  if (isSceneDetail && customSceneLicenceCents > 0) {
    cards.push(`<div class="card"><span>${escapeHtmlText(summary.scene_custom_licence_label || SCENE_CUSTOM_LICENCE_LABEL)}</span><b>${formatEur(centsToEur(customSceneLicenceCents))}</b></div>`);
  }
  const button = buyHref ? `<a class="button buy-now" href="${escapeHtmlText(buyHref)}">${finalPriceCents > 0 ? "Buy Now" : "Licence Now"}</a>` : "";
  cards.push(`<div class="card final-price"><span>Final Price</span><b>${formatEur(centsToEur(finalPriceCents))}</b>${button}</div>`);
  return cards.join("");
}

function productLicenceNotesHtml() {
  return `
<section class="panel product-licence-notes">
<h2>Licensing Information</h2>
<p>Licenced tiles remain available to your Planetka account permanently.</p>
...
function upsellRowFromServerCard(card) {
  const pack = card && card.region_pack || {};
  const summary = card && card.summary || null;
  const quoteId = String(card && card.quote_id || "").trim();
  const ready = Boolean(summary && quoteId);
  const partialCount = Math.max(0, Number.parseInt(
    summary && (summary.partial_licence_tiles ?? summary.partial_licence_tile_count) || 0,
    10,
  ) || 0);
  const partialCents = integerCents(summary && summary.partial_licence_credit_cents);
  const alreadyCents = integerCents(summary && summary.already_licenced_deduction_cents) + partialCents;
  const group = regionProductCatalogGroup(pack);
  return {
    id: String(pack && pack.id || card && card.asset_id || "").trim(),
    quote_id: quoteId,
    ready,
    name: String(pack && pack.name || "Data Pack").trim() || "Data Pack",
    group_label: group.label,
    data_size_text: String(summary && summary.data_size_text || pack && pack.data_size_text || formatDataSize(summary && summary.data_size_bytes || pack && pack.data_size_bytes || 0)),
    new_tiles: ready ? Math.max(0, Number.parseInt(summary.new_tiles || 0, 10) - partialCount) : 0,
    total_tiles: ready ? Math.max(0, Number.parseInt(summary.total_tiles || 0, 10) || 0) : 0,
    already_licenced_tiles: ready ? Math.max(0, Number.parseInt(summary.already_licenced_tiles || 0, 10) || 0) + partialCount : 0,
    already_licenced_deduction_cents: ready ? alreadyCents : 0,
    discount_percent: ready ? Math.max(0, Number.parseInt(summary.discount_percent || 0, 10) || 0) : 0,
    discount_cents: ready ? integerCents(summary.discount_cents) : 0,
    full_price_cents: ready ? integerCents(summary.full_price_cents) : 0,
    price_cents: ready ? integerCents(summary.price_cents) : 0,
  };
}

function upsellCatalogTableHtml(upsells, token) {
  const rows = (Array.isArray(upsells) ? upsells : []).map(upsellRowFromServerCard).filter((row) => row.id);
  if (!rows.length) {
    return "";
  }
  const tokenParam = escapeHtmlText(encodeURIComponent(String(token || "")));
  const rowHtml = rows.map((row) => {
    const packId = escapeHtmlText(encodeURIComponent(row.id));
    const quoteId = escapeHtmlText(encodeURIComponent(row.quote_id));
    const quoteParam = quoteId ? `&quote_id=${quoteId}` : "";
    const pending = `<span class="pending">Price updating</span>`;
    const mapHref = `/credits/region-pack-map?token=${tokenParam}&region_pack_id=${packId}&catalog=1${quoteParam}`;
    const buy = row.ready
      ? `<a class="button" href="/credits/region-pack-checkout?token=${tokenParam}&region_pack_id=${packId}&catalog=1${quoteParam}">Buy</a>`
      : `<span class="button disabled">Buy</span>`;
    const map = row.ready
      ? `<a class="button secondary" href="${mapHref}">Detailed Map</a>`
      : `<span class="button secondary disabled">Detailed Map</span>`;
    const name = row.ready
      ? `<a href="${mapHref}">${escapeHtmlText(row.name)}</a>`
      : `<b>${escapeHtmlText(row.name)}</b>`;
    return `<tr>
<td>${name}<div class="muted small">${escapeHtmlText(row.group_label)}</div></td>
<td>${row.ready ? formatInteger(row.new_tiles) : pending}</td>
<td>${escapeHtmlText(row.data_size_text)}</td>
<td class="price">${row.ready ? formatEur(centsToEur(row.price_cents)) : pending}</td>
<td class="catalog-actions">${map}</td>
<td class="catalog-actions">${buy}</td>
</tr>`;
  }).join("");
  return `<section class="panel">
<h2>Other Options</h2>
<div class="upsells"><table>
<colgroup><col style="width:36%"><col style="width:12%"><col style="width:16%"><col style="width:16%"><col style="width:12%"><col style="width:8%"></colgroup>
<thead><tr><th>Data Pack</th><th>New Tiles</th><th>Data Size</th><th>Final Price</th><th>Detailed Map</th><th>Buy</th></tr></thead>
<tbody>${rowHtml}</tbody>
</table></div>
</section>`;
}

function paymentSuccessConfirmationHtml(data = {}) {
  const title = String(data.title || "Payment successful").trim() || "Payment successful";
  const message = String(data.message || "Thank you. Your Planetka purchase is complete.").trim();
  const detailHref = String(data.detail_href || "").trim();
  const detailLabel = String(data.detail_label || "Detailed Map").trim() || "Detailed Map";
  const catalogHref = String(data.catalog_href || "").trim();
  const detailSection = detailHref
    ? `<section class="panel">
<h2>Purchase Details</h2>
<p>View detailed map of your purchase:</p>
<a class="catalog-back" href="${escapeHtmlText(detailHref)}">${escapeHtmlText(detailLabel)}</a>
</section>`
    : "";
  const catalogSection = catalogHref
    ? `<section class="panel">
<a class="catalog-back" href="${escapeHtmlText(catalogHref)}">Product Catalog</a>
</section>`
    : "";
  return `<!doctype html>
<html lang="en">
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Planetka Payment Complete</title>
<style>${REGION_PACK_SHARED_CSS}</style>
</head>
<body>
<main>
...
${success ? `<section class="panel"><h2>${escapeHtmlText(success.title || "Payment successful")}</h2><p>${escapeHtmlText(success.message || "Your Planetka purchase has been processed.")}</p></section>` : ""}
<div class="page-heading">
<a class="catalog-back" href="/credits/region-pack-catalog?token=${escapeHtmlText(encodeURIComponent(String(data && data.token || "")))}">&larr; Product Catalog</a>
<h1 id="pageTitle">${escapeHtmlText(pageTitle)}</h1>
</div>
<section id="cards" class="cards">${regionPackStaticCardsHtml(data)}</section>
<section class="panel">
<div class="map-shell">
<div id="mapToolbar" class="map-toolbar-overlay">
<span class="map-toolbar-label">Zoom level</span>
...
<h2>${escapeHtmlText(String(data && data.upsell_title || "Other Options"))}</h2>
<div id="upsellGrid" class="upsells"></div>
</section>
${isSceneDetail ? "" : productLicenceNotes}
</main>
...
const fmtSize=(v)=>{const b=Math.max(0,Math.round(Number(v||0)||0));const mb=b/1000000;if(mb<1000)return fmtMoney(mb)+" MB";const gb=b/1000000000;if(gb<1000)return fmtMoney(gb)+" GB";return fmtMoney(b/1000000000000)+" TB"};
const token=encodeURIComponent(DATA.token||"");
  let ROWS=[];let loading=false;let loadedAll=false;let nextOffset=0;const FIRST_LIMIT=20;const NEXT_LIMIT=20;
const catalogColgroup="<colgroup><col style=\\"width:36%\\"><col style=\\"width:12%\\"><col style=\\"width:16%\\"><col style=\\"width:16%\\"><col style=\\"width:12%\\"><col style=\\"width:8%\\"></colgroup>";
const catalogHead="<thead><tr><th>Data Pack</th><th>New Tiles</th><th>Data Size</th><th>Final Price</th><th>Detailed Map</th><th>Buy</th></tr></thead>";
function escapeCell(value){return String(value||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}
	function rowHtml(row){const id=encodeURIComponent(row.id||"");const quote=encodeURIComponent(row.quote_id||"");const ready=String(row.quote_status||"ready")==="ready"&&!row.price_pending&&quote;const quoteParam=quote?"&quote_id="+quote:"";const partialCount=Number(row.partial_licence_tiles??row.partial_licence_tile_count??0);const newTiles=Math.max(0,Number(row.new_tiles||0)-partialCount);const pending="<span class=\\"pending\\">"+escapeCell(row.status_label||"Price updating")+"</span>";const mapHref="/credits/region-pack-map?token="+token+"&region_pack_id="+id+"&catalog=1"+quoteParam;const name=ready?"<a href=\\""+mapHref+"\\">"+escapeCell(row.name||"Data Pack")+"</a>":"<b>"+escapeCell(row.name||"Data Pack")+"</b>";const details=ready?"<a class=\\"button secondary\\" href=\\""+mapHref+"\\">Detailed Map</a>":"<span class=\\"button secondary disabled\\">Detailed Map</span>";return "<tr>"
+"<td>"+name+"<div class=\\"muted small\\">"+escapeCell(row.group_label||"")+"</div></td>"
+"<td>"+(ready?fmtNumber(newTiles):pending)+"</td>"
+"<td>"+escapeCell(row.data_size_text||fmtSize(row.data_size_bytes))+"</td>"
+"<td class=\\"price\\">"+(ready?fmt(row.price_eur):pending)+"</td>"
+"<td class=\\"catalog-actions\\">"+details+"</td>"
	+"<td class=\\"catalog-actions\\">"+(ready?"<a class=\\"button\\" href=\\"/credits/region-pack-checkout?token="+token+"&region_pack_id="+id+"&catalog=1"+quoteParam+"\\">Buy</a>":"<span class=\\"button disabled\\">Buy</span>")+"</td>"
+"</tr>"}
...
function loadStatusHtml(label,loaded,total){return "<span class=\\"load-status\\"><span>"+escapeCell(label)+"</span><span class=\\"loaded\\">"+fmtNumber(loaded)+"</span><span>/</span><span class=\\"total\\">"+fmtNumber(total)+"</span></span>"}
function render(){const filter=String(document.getElementById("filter").value||"").trim().toLowerCase();let shown=0;const total=Number(DATA.total_packs||ROWS.length||0);const rows=ROWS.filter(row=>!filter||String(row.name||"").toLowerCase().includes(filter)||String(row.group_label||"").toLowerCase().includes(filter)||String(row.id||"").toLowerCase().includes(filter));const body=groupRows(rows).map(group=>{const groupRows=group.rows||[];if(!groupRows.length)return "";shown+=groupRows.length;return "<tr class='catalog-group-row'><td colspan='6'>"+escapeCell(group.label)+"</td></tr>"+groupRows.map(rowHtml).join("")}).join("");let html=body?"<table>"+catalogColgroup+catalogHead+"<tbody>"+body+"</tbody></table>":"";if(!html){html="<div class='empty'>"+(loadedAll?"No data packs match this search.":loadStatusHtml("Loading",ROWS.length,total))+"</div>"}document.getElementById("catalog").innerHTML=html;document.getElementById("count").innerHTML=loadStatusHtml(loadedAll?"Loaded":"Loading",ROWS.length,total);}
...
export async function unlockTilesForSession(db, userId, qualityMode, tileKeys, resolveId, deps, options = {}) {
  ...
  if (insertedTiles.length > 0) {
    await invalidateUserEntitlementCaches(db, safeUserId, deps);
  ...
  if (insertedTiles.length > 0) {
    await touchUserPricingVersion(db, safeUserId, deps);
...
export async function grantRegionPackEntitlements(db, userId, regionPackId, stripeSessionId, amountPaidEur, deps, userEmail = "", stripePaymentIntentId = "", options = {}) {
  ...
  if (insertedTiles.length > 0) {
    ...
    for (const coveredProductId of fullyCoveredRegionPackProductIdsForPurchasedProduct(String(product.id || ""))) {
      const coveredProduct = regionProductById(coveredProductId);
      const coveredQuote = await materializePurchasedRegionPackQuoteNow(db, safeUserId, coveredProduct, deps, null);
      if (coveredQuote && coveredQuote.ok) {
        immediateReadyProductIds.push(coveredProductId);
      } else {
        console.warn(
          "planetka.covered_region_pack_quote_immediate_failed",
          JSON.stringify({
            user_id: safeUserId,
            source_region_pack_id: String(product.id || ""),
            covered_region_pack_id: coveredProductId,
            error: String(coveredQuote && coveredQuote.error || "covered_region_pack_quote_failed"),
          }),
        );
      }
    }
...
async function createStripeCheckoutSession(env, params, deps) {
  ...
  body.set(
    ...
    checkoutMetadataValue(params.productDescription || "Full Quality Planetka Earth texture data for Blender. Commercial licence included.", 1000),
  );
  body.set(
...
function stripeCheckoutPriceLine(label, value) {
  const safeLabel = String(label || "").trim();
  if (!safeLabel) {
    return "";
  }
...
function stripeCheckoutPurchaseDescription(kind, estimate, policy, options = {}) {
  const parts = [];
  const newTiles = Math.max(0, Number.parseInt(estimate && estimate.paid_tile_count || 0, 10) || 0);
  const alreadyTiles = estimateAlreadyLicencedTileCount(estimate)
    + Math.max(0, Number.parseInt(estimate && estimate.partial_licence_tile_count || 0, 10) || 0);
  const totalTiles = Math.max(0, Number.parseInt(estimate && estimate.tile_count || 0, 10) || 0);
  const tilePrice = normalizeCreditAmount(policy && policy.scene_tile_price_eur);
  const licencePrice = normalizeCreditAmount(
    policy && (
      policy.custom_animation_licence_eur
      ?? policy.custom_scene_licence_eur
      ?? policy.custom_scene_licence_eur
    ),
  );
  const totalPrice = normalizeCreditAmount(policy && (policy.scene_payable_eur ?? policy.animation_payable_eur));
  const segmentCount = Math.max(0, Number.parseInt(options && options.segment_count || 0, 10) || 0);
  const prefix = String(kind || "").trim().toLowerCase() === "animation" && segmentCount > 0
    ? `${segmentCount} animation resolve${segmentCount === 1 ? "" : "s"}`
    : "Current Blender camera view";
  parts.push(prefix);
  parts.push(stripeCheckoutPriceLine("New Full Quality tiles", newTiles));
  if (alreadyTiles > 0) {
    parts.push(stripeCheckoutPriceLine("Already licenced tiles", alreadyTiles));
  }
  if (totalTiles > 0) {
    parts.push(stripeCheckoutPriceLine("Total requested tiles", totalTiles));
  }
  parts.push(stripeCheckoutPriceLine("Texture data", formatEur(tilePrice)));
  if (licencePrice > 0) {
    parts.push(stripeCheckoutPriceLine(String(policy && (policy.custom_animation_licence_label || policy.scene_custom_licence_label) || "Licence"), formatEur(licencePrice)));
  }
  parts.push(stripeCheckoutPriceLine("Total", formatEur(totalPrice)));
...
export async function handleCreditCheckout(request, env, deps) {
  ...
  if (option === "animation" || option === "animation_tiles") {
    ...
    const session = await createStripeCheckoutSession(
      env,
      {
	        ...
	        productDescription: stripeCheckoutPurchaseDescription("animation", estimate, animationPolicy, {
	          segment_count: Math.max(0, Number.parseInt(keyResult.segment_count || 0, 10) || 0),
	        }),
	        metadata: {
  ...
  if (option === "region_pack" || option === "broader_pack") {
    ...
    if (!regionPackCheckoutQuoteIsCurrent(quote, userId, product, account)) {
      await enqueueUserProductQuoteJob(db, productQuoteLookupUserIdForAccount(userId, account), String(product.id || ""), pricingSettingsCacheKey(), accountEntitlementVersion(account), deps, {
        jobRound: 0,
        priority: 0,
        triggerType: "checkout_region_pack_stale_quote_blocked",
        staleReason: "checkout_quote_version_mismatch",
        fastTrack: true,
      });
      return deps.json(
        {
  ...
  const session = await createStripeCheckoutSession(
    env,
    {
	      ...
	      productDescription: stripeCheckoutPurchaseDescription("scene", estimate, scenePolicy),
	      metadata: {
...
function fullQualityPriceBreakdownHtml({
  ...
  let html = `<p class="muted">Full Price: ${formatEur(full)}<br>`;
  if (already > 0) {
    html += `Licenced: - ${formatEur(already)}<br>`;
  }
  if (partial > 0) {
    html += `Partially licenced: - ${formatEur(partial)}<br>`;
  }
  if (discount > 0) {
    html += `Volume Discount (${pct}%): - ${formatEur(discount)}<br>`;
  }
  html += `Final Price: ${formatEur(final)}</p>`;
  return html;
}
...
function regionPackPaymentChoiceHtml(data) {
  ...
  const stripeButton = stripeAvailable
    ? `<form method="post" action="/credits/region-pack-checkout"><input type="hidden" name="token" value="${token}"><input type="hidden" name="region_pack_id" value="${regionPackId}">${catalogInput}${quoteInput}<input type="hidden" name="method" value="stripe"><button class="button" type="submit">Pay Now (${formatEur(priceEur)})</button></form>`
    : `<button class="button disabled" type="button" disabled>Payment gateway unavailable below ${formatEur(STRIPE_MIN_CHECKOUT_AMOUNT_CENTS / 100)}</button>`;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<body>
	...
	<section class="panel">
			...
			<div class="summary">
		<div class="card"><span>New Tiles / Total Tiles</span><b>${formatInteger(Math.max(0, unlicencedTileCount - partialLicenceCount))} / ${formatInteger(summary.total_tiles)}</b></div>
		<div class="card"><span>Full Price</span><b>${formatEur(fullPriceEur)}</b></div>
		${displayedAlreadyLicencedDeductionEur > 0 ? `<div class="card"><span>Already Licenced</span><b>${formatInteger(displayedAlreadyLicencedCount)} tiles\n-${formatEur(displayedAlreadyLicencedDeductionEur)}</b></div>` : ""}
		${discountEur > 0 ? `<div class="card"><span>Volume Discount</span><b>${discountPercent}%\n-${formatEur(discountEur)}</b></div>` : ""}
	<div class="card"><span>Final Price</span><b>${formatEur(priceEur)}</b></div>
		</div>
	<div class="actions">
	${stripeButton}
	</div>
...
export async function handleCreditRegionPackCheckoutFromToken(request, env, deps) {
  ...
  if (!regionPackCheckoutQuoteIsCurrent(quote, userId, product, account)) {
    await enqueueUserProductQuoteJob(db, productQuoteLookupUserIdForAccount(userId, account), String(product.id || ""), pricingSettingsCacheKey(), accountEntitlementVersion(account), deps, {
      jobRound: 0,
      priority: 0,
      triggerType: "region_pack_checkout_stale_quote_blocked",
      staleReason: "region_pack_checkout_quote_version_mismatch",
      fastTrack: true,
    });
    return withEndpointTiming(html(
      `<!doctype html><title>Planetka Data Pack</title><h1>Data pack price is updating.</h1><p>Please wait a few moments, then reload this page or reopen the data pack from Blender.</p>`,
...
export async function handleCreditRegionPackMap(request, env, deps) {
  ...
  if (quoteStatus === "ready") {
    const materializedMapState = await parseUserProductMapStatePayload(quoteRow, env, { allowNonReady: false });
    if (userProductMapStateAllLevelsReady(materializedMapState)) {
      mapState = materializedMapState;
      mapStateStatus = "ready_precomputed";
    } else {
      mapState = await buildRegionPackOnDemandMapState(db, product, quote, deps);
      mapStateStatus = "ready_on_demand";
      await enqueueUserProductMapPrewarmJobIfNeeded(db, quoteUserId, product, pricingVersion, entitlementVersion, deps, {
        quoteRow,
        summary: quote && quote.summary,
        env,
        triggerType: "product_page_map_state_prewarm",
        staleReason: "product_page_map_state_not_precomputed",
        jobRound: REGION_PACK_MAP_PREWARM_JOB_ROUND,
        priority: regionProductMapPrewarmPriority(product, quote && quote.summary),
      });
    }
  }
  if (quoteStatus !== "ready") {
...
export async function handleCreditRegionPackMiniMap(request, env, deps) {
  ...
  const miniBounds = regionMapBounds(product, GENERATED_REGION_PACK_DETAILS[String(product && product.id || "")] || {}, rows);
  return withEndpointTiming(deps.json({
    ok: true,
    catalog_version: REGION_PACK_CATALOG_VERSION,
...
export async function handleCreditRegionPackMapAsset(request, env, deps) {
  ...
  if (!payload || typeof payload !== "object") {
    return withEndpointTiming(deps.json({ ok: false, error: "region_pack_map_asset_invalid" }, 500, env), timing, env, { region_pack_id: regionPackId });
  }
  payload.region_pack = {
    ...(payload.region_pack && typeof payload.region_pack === "object" ? payload.region_pack : {}),
    ...regionProductPublicPayload(product),
  };
...
export async function handleCreditSceneCheckout(request, env, deps) {
  ...
  const session = await createStripeCheckoutSession(
    env,
    {
      ...
      metadata: {
        ...
        planetka_tile_keys_json: JSON.stringify(normalizeTileKeys(tileKeys)),
  ...
  const checkoutAmountRecord = await storeStripeCheckoutAmountRecord(
    db,
    {
      ...
      currency: "eur",
      metadata: {
        tile_set_token: token,
        tile_keys: normalizeTileKeys(tileKeys),
        paid_tile_count: Math.max(0, Number.parseInt(estimate && estimate.paid_tile_count || 0, 10) || 0),
        price_eur: scenePolicy.scene_payable_eur.toFixed(2),
      },
    },
    deps,
  );
  if (checkoutAmountRecord && checkoutAmountRecord.error) {
    return withEndpointTiming(html(
      `<!doctype html><title>Planetka Scene Payment</title><h1>Checkout could not be confirmed.</h1><p>${escapeHtmlText(checkoutAmountRecord.error)}</p>`,
      500,
      env,
    ), timing, env, { error: checkoutAmountRecord.error });
  }
...
export async function handleCreditPaymentSuccess(request, env, deps) {
  ...
  try {
    ...
    console.warn(
      "stripe.payment_success_load_deferred",
      JSON.stringify({ session_id: sessionId, error: error && error.message || String(error || "unknown_error") }),
    );
  }
  ...
  const paymentSuccessTitle = amountPaidEur > 0
    ? `Payment of ${formatEur(amountPaidEur)} successful`
    : "Payment successful";

  const message = "Thank you for your purchase. The data is already downloading in Blender. Enjoy working on your project with Full Quality Planetka textures.";
  ...
  if (userId) {
    try {
      const regionPackId = purchaseType === "region_pack"
        ? String(purchase && purchase.region_pack_id || metadata.planetka_region_id || "").trim()
        : "";
      const detailProduct = regionPackId ? regionProductById(regionPackId) : null;
      const tokenProduct = detailProduct || regionProductById("world");
      if (tokenProduct) {
        const tokenResult = await createRegionPackDetailTokenForUser(db, userId, String(tokenProduct.id || ""), env, deps);
        const tokenParam = encodeURIComponent(String(tokenResult && tokenResult.token || ""));
        if (tokenParam) {
          catalogHref = `/credits/region-pack-catalog?token=${tokenParam}`;
          if (detailProduct) {
            detailHref = `/credits/region-pack-map?token=${tokenParam}&region_pack_id=${encodeURIComponent(String(detailProduct.id || ""))}&catalog=1`;
            detailLabel = `${String(detailProduct.name || "Data Pack").trim() || "Data Pack"} Detailed Map`;
          }
        }
      }
    } catch (error) {
      console.warn(
        "stripe.payment_success_link_unavailable",
        JSON.stringify({ session_id: sessionId, user_id: userId, error: error && error.message || String(error || "unknown_error") }),
      );
    }
  }

  return html(
    paymentSuccessConfirmationHtml({
...
function accountPageCoverageMapHtml({ worldUnlocked, tileRows, totalTileCount, visibleTileLimit }) {
  ...
  return `<section class="panel">
    ...
    <svg id="accountCoverageMap" class="coverage-map" viewBox="0 0 360 180" role="img" aria-label="Planetka licenced tile coverage map" preserveAspectRatio="xMidYMid meet">
      <image href="/maps/region-pack-map-background.jpg?v=${safeRevision}" x="0" y="0" width="360" height="180" preserveAspectRatio="none"></image>
    </svg>
