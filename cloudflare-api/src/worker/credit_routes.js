...
const REGION_PACK_MAP_ASSET_REVISION = `${REGION_PACK_CATALOG_VERSION}:outline-v4-product-bg-wt-blue-v4-partial-dateline-v7-admin-labels-v1-success-upsell-v1-catalog-flat-v1-price-breakdown-v1-hover-breakdown-v1-summary-partial-v1-pricing-runtime-v1`;
...
export async function setRuntimePricingSettings(db, values = {}, adminUserId = "", deps = {}) {
  ...
  const settings = normalizeRuntimePricingSettings(values, {});
  await ensurePricingSettingsTable(db, deps);
...
<h1>${isSceneDetail ? "Full Quality for This Scene" : `${escapeHtmlText(name)} Full Quality Pack`}</h1>
	${success ? `<section class="panel"><h2>${escapeHtmlText(success.title || "Payment successful")}</h2><p>${escapeHtmlText(success.message || "Your Planetka purchase has been processed.")}</p></section>` : ""}
		...
		<div class="card"><span>Already Licenced</span><b>${alreadyLicencedTiles} tiles (-€${alreadyLicencedDeductionEur.toFixed(2)})</b></div>
...
<body>
	...
	<section class="panel">
		...
		<div class="card"><span>Already Licenced</span><b>${displayedAlreadyLicencedCount} tiles (-€${displayedAlreadyLicencedDeductionEur.toFixed(2)})</b></div>
...
export async function handleCreditRegionPackCatalog(request, env, deps) {
  ...
  return withEndpointTiming(html(
    regionPackStaticCatalogHtml({
      ...
      map_asset_revision: REGION_PACK_MAP_ASSET_REVISION,
      price_coefficient: fullQualityPriceCoefficient(),
      region_pack_discount_min_percent: Number(activePricingSettings().region_pack_discount_min_percent || 0),
      region_pack_discount_max_percent: Number(activePricingSettings().region_pack_discount_max_percent || 0),
      token,
