...
export async function handleAdminUserReleasePreviewHold(request, env, deps) {
  const auth = await deps.requireAnalyticsAdmin(request, env);
  if (auth.error) {
    return auth.error;
  }
  ...
  await deps.dbRun(
    ...
    `
      UPDATE users
      SET
        preview_fair_usage_hold_at = NULL,
        preview_fair_usage_hold_reason = NULL,
        preview_fair_usage_hold_details_json = NULL
      WHERE id = ?
    `,
    [targetUserId],
  );
  try {
    console.log(
      "admin.preview_fair_usage_hold_released",
      JSON.stringify({
        user_id: targetUserId,
        user_email: targetEmail,
        admin_email: deps.normalizeEmail(adminUser && adminUser.email || ""),
...
export async function handleAdminUserSetPreviewHold(request, env, deps) {
  const auth = await deps.requireAnalyticsAdmin(request, env);
  if (auth.error) {
    return auth.error;
  }
  ...
  const targetEmail = deps.normalizeEmail(target.user.email || "");
  const now = deps.nowIso();
  const reason = String(body && body.reason || "admin_preview_review").trim().slice(0, 160) || "admin_preview_review";
  const details = {
    reason,
    admin_email: deps.normalizeEmail(adminUser && adminUser.email || ""),
    held_at: now,
  ...
  await deps.dbRun(
    ...
    `
      UPDATE users
      SET
        preview_fair_usage_hold_at = ?,
        preview_fair_usage_hold_reason = ?,
        preview_fair_usage_hold_details_json = ?
      WHERE id = ?
    `,
    [now, reason, JSON.stringify(details), targetUserId],
  );
  try {
    console.log(
      "admin.preview_fair_usage_hold_set",
      JSON.stringify({
        user_id: targetUserId,
        user_email: targetEmail,
        admin_email: deps.normalizeEmail(adminUser && adminUser.email || ""),
  ...
  return deps.json(
    {
      ok: true,
      action: "set_preview_hold",
      user_id: targetUserId,
      user_email: targetEmail,
      preview_fair_usage_hold_at: now,
      preview_fair_usage_hold_reason: reason,
    },
    200,
    env,
  );
