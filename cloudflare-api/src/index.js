  ...
  async scheduled(controller, env, ctx) {
    ...
    ctx.waitUntil((async () => {
      try {
        ...
        const maintenance = runHourlyMaintenance
          ? await runScheduledMaintenanceJobs(
            db,
            env,
            runStartedAt,
          ...
          : null;
        const analyticsSnapshotSummary = runHourlyMaintenance
          ? await buildAnalyticsSnapshotMatrix(
            db,
            env,
            ADMIN_ANALYTICS_DEPS,
          )
          : null;
        const analyticsUsersSnapshot = runHourlyMaintenance
          ? await buildAnalyticsUsersSnapshot(
            db,
            env,
            ADMIN_ANALYTICS_DEPS,
          )
        ...
        console.log(
          ...
          JSON.stringify({
            ...
            maintenance_ran: runHourlyMaintenance,
            ...(maintenance && maintenance.summary ? maintenance.summary : {}),
            production_alert_summary: maintenance && maintenance.alertSummary || null,
            monthly_cost_summary: maintenance && maintenance.monthlyCostSummary || null,
            quote_job_summary: quoteJobSummary,
