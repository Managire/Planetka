...
def write_js(path: Path, catalog: dict, include_details: bool = False):
    ...
    for product_id, tile_keys in tile_keys_by_product.items():
        tile_lines.append(f"  {json.dumps(product_id)}: [")
        for key in tile_keys:
            tile_lines.append(f"    {json.dumps(key)},")
        tile_lines.append("  ],")
    tile_lines.append("};")
    tile_lines.append("")
    tile_lines.append(f"export const GENERATED_REGION_PACK_TILE_REFS = {json.dumps(tile_refs_by_product, ensure_ascii=True, separators=(',', ':'))};")
    tile_lines.append("")
    tile_lines.append(f"export const GENERATED_REGION_PACK_TILE_GROSS_CENTS = {json.dumps(catalog.get('tile_gross_cents') or {}, ensure_ascii=True, separators=(',', ':'))};")
    detail_payload = {}
    if include_details:
        for payload in pack_payloads:
    ...
    tile_lines.append("")
    tile_lines.append(f"export const GENERATED_REGION_PACK_DETAILS = {json.dumps(detail_payload, ensure_ascii=True, separators=(',', ':'))};")
    tile_lines.append("")
