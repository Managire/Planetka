...
class Pipeline:
    ...
    @staticmethod
    def _imgbuf_has_error(buf: oiio.ImageBuf) -> bool:
        has_error_attr = getattr(buf, "has_error", False)
        if callable(has_error_attr):
            return bool(has_error_attr())
        return bool(has_error_attr)
    ...
    def _write_scalar_tif(self, path: Path, image: np.ndarray) -> None:
        ...
        ok = cv2.imwrite(str(tmp), data, [cv2.IMWRITE_TIFF_COMPRESSION, 5])  # 5 = LZW
        if not ok:
            raise RuntimeError(f"failed writing tif {tmp}")
        tmp.replace(path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Resumable PO-only rebuild for higher-z/higher-d tiles "
            "using direct-from-lowest-d policy with immediate Cloud upload."
        )
    )
    parser.add_argument("--assets-root", default="/Volumes/SSDA/Planetka Assets", help="Root with PO folder")
    parser.add_argument(
        "--state-dir",
        default="/Volumes/SSDA/Planetka Assets/.rebuild_po_state",
