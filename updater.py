...
def _run_update_install_worker(force=False):
    ...
    try:
        ...
        state["available_update"] = None
        state["last_apply_at"] = int(time.time())
        state["last_applied_version"] = str(refreshed or latest_version or "").strip()
        state["current_version"] = str(refreshed or "").strip()
        _save_state(state)
        _set_runtime(
            checking=False,
            ...
            update_ready=False,
            last_check_at=now_ts,
            message=f"Update {str(refreshed or latest_version or '').strip()} installed. Restart Blender to load it.",
            last_error="",
            phase="installed",
            downloaded_bytes=0,
            download_total_bytes=0,
        )
        logger.info("Planetka updater: installed version %s", str(refreshed or latest_version or "").strip())
    except urllib.error.HTTPError as exc:
        state["last_check_at"] = now_ts
        state["last_error"] = f"network:{exc}"
        _save_state(state)
